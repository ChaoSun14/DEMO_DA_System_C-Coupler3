module decomp_init_mod_2
    integer, public, allocatable :: local_grid_size(:)
    integer, public, allocatable :: local_grid_index(:,:)
contains
    subroutine decomp_init_da
        use spmd_init_mod_2, only: npes, mytask_id, ier
        use grid_init_mod_2, only: nlat, nlon
        use parse_namelist_mod_2, only : decomp_type_id
        implicit none
include 'mpif.h'
        integer :: i, j, sum
