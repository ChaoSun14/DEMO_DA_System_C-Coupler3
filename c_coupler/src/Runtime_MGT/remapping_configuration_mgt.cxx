...
{    
    ...
    num_wgts = 0;
    checksum_src_mask = -1;
	checksum_src_center_lon = -1;
	checksum_src_center_lat = -1;
    checksum_dst_mask = -1;
	checksum_dst_center_lon = -1;
	checksum_dst_center_lat = -1;
    src_grid_size = -1;
    dst_grid_size = -1;
}
...
{    
    ...
    read_data_from_array_buffer(&checksum_dst_center_lat, sizeof(long), array, *buffer_content_iter, true);
    read_data_from_array_buffer(&checksum_dst_center_lon, sizeof(long), array, *buffer_content_iter, true);
    ...
    read_data_from_array_buffer(&checksum_src_center_lat, sizeof(long), array, *buffer_content_iter, true);
    read_data_from_array_buffer(&checksum_src_center_lon, sizeof(long), array, *buffer_content_iter, true);
...
long H2D_remapping_wgt_file_info::get_grid_field_checksum_value(const char *field_name, IO_netcdf *netcdf_file_object, int grid_size, MPI_Comm comm, bool is_root_proc)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, words_are_the_same(data_type, DATA_TYPE_INT) || words_are_the_same(data_type, DATA_TYPE_DOUBLE), "Software error in H2D_remapping_wgt_file_info::get_grid_field_checksum_value");
    checksum = calculate_checksum_of_array(data_buffer, field_size, get_data_type_size(data_type), NULL, MPI_COMM_NULL);
...
void H2D_remapping_wgt_file_info::read_in_all_checksums(Comp_comm_group_mgt_node *comp_node)
{
	long checksum_arrays[8];
	...
	if (comp_node->get_current_proc_local_id() == 0) {
		...
		netcdf_file_object->get_file_field_attribute(NULL, "mask_checksum_a", (char*)(&checksum_arrays[0]), data_type);
		netcdf_file_object->get_file_field_attribute(NULL, "mask_checksum_b", (char*)(&checksum_arrays[1]), data_type);
		netcdf_file_object->get_file_field_attribute(NULL, "xc_checksum_a", (char*)(&checksum_arrays[2]), data_type);
		netcdf_file_object->get_file_field_attribute(NULL, "xc_checksum_b", (char*)(&checksum_arrays[3]), data_type);
		netcdf_file_object->get_file_field_attribute(NULL, "yc_checksum_a", (char*)(&checksum_arrays[4]), data_type);
		netcdf_file_object->get_file_field_attribute(NULL, "yc_checksum_b", (char*)(&checksum_arrays[5]), data_type);
		checksum_arrays[6] = netcdf_file_object->get_dimension_size("n_a", MPI_COMM_NULL, true);
		checksum_arrays[7] = netcdf_file_object->get_dimension_size("n_b", MPI_COMM_NULL, true);
		delete netcdf_file_object;
	}
	EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Bcast(checksum_arrays, 8, MPI_LONG, 0, comp_node->get_comm_group()) == MPI_SUCCESS);
	checksum_src_mask = checksum_arrays[0];
	checksum_dst_mask = checksum_arrays[1];
	checksum_src_center_lon = checksum_arrays[2];
	checksum_dst_center_lon = checksum_arrays[3];
	checksum_src_center_lat = checksum_arrays[4];
	checksum_dst_center_lat = checksum_arrays[5];
	src_grid_size = (int) checksum_arrays[6];
	dst_grid_size = (int) checksum_arrays[7];
}
...
bool H2D_remapping_wgt_file_info::match_H2D_remapping_wgt(Original_grid_info *src_original_grid, Original_grid_info *dst_original_grid)
{
	...
	read_in_all_checksums(comp_comm_group_mgt_mgr->search_global_node(dst_original_grid->get_comp_id()));
    ...
    if (dst_grid_size != dst_original_grid->get_H2D_sub_CoR_grid()->get_grid_size())
        return false;
    get_wgt_file_grid_checksum(dst_original_grid->get_comp_id(), "mask_a", "mask_checksum_a", src_grid_size, checksum_src_mask);
    if (src_original_grid->get_checksum_H2D_mask() != this->checksum_src_mask)
        return false;
    get_wgt_file_grid_checksum(dst_original_grid->get_comp_id(), "mask_b", "mask_checksum_b", dst_grid_size, checksum_dst_mask);
    if (dst_original_grid->get_checksum_H2D_mask() != this->checksum_dst_mask)
        return false;
    get_wgt_file_grid_checksum(dst_original_grid->get_comp_id(), "xc_a", "xc_checksum_a", src_grid_size, checksum_src_center_lon);
    if (src_original_grid->get_checksum_H2D_center_lon() != this->checksum_src_center_lon)
        return false;
    get_wgt_file_grid_checksum(dst_original_grid->get_comp_id(), "xc_b", "xc_checksum_b", dst_grid_size, checksum_dst_center_lon);
    if (dst_original_grid->get_checksum_H2D_center_lon() != this->checksum_dst_center_lon)
        return false;
    get_wgt_file_grid_checksum(dst_original_grid->get_comp_id(), "yc_a", "yc_checksum_a", src_grid_size, checksum_src_center_lat);
    if (src_original_grid->get_checksum_H2D_center_lat() != this->checksum_src_center_lat)
        return false;
    get_wgt_file_grid_checksum(dst_original_grid->get_comp_id(), "yc_b", "yc_checksum_b", dst_grid_size, checksum_dst_center_lat);
    if (dst_original_grid->get_checksum_H2D_center_lat() != this->checksum_dst_center_lat)
        return false;
...
bool H2D_remapping_wgt_file_info::get_wgt_file_grid_checksum(int comp_id, const char *data_label, const char *checksum_label, int grid_size, long &checksum_value)
{
	long checksum_from_file = -1;
	char data_type[NAME_STR_SIZE];
    Comp_comm_group_mgt_node *comp_node = comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id, false, "in H2D_remapping_wgt_file_info::get_wgt_file_grid_checksum");
	...
	if (comp_node->get_current_proc_local_id() == 0) {
    	IO_netcdf *netcdf_file_object = new IO_netcdf("remapping weights file for H2D interpolation", wgt_file_name, "r", false);    
	    checksum_value = get_grid_field_checksum_value(data_label, netcdf_file_object, grid_size, MPI_COMM_NULL, true);
		netcdf_file_object->put_global_attr(checksum_label, &checksum_value, DATA_TYPE_LONG, DATA_TYPE_LONG, 1);
	    delete netcdf_file_object;
	}
	
	EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Bcast(&checksum_value, 8, MPI_LONG, 0, comp_node->get_comm_group()) == MPI_SUCCESS);
...
void H2D_remapping_wgt_file_info::read_grid_size(int comp_id, const char *label, int &grid_size)
{
    int i;
    char data_type[NAME_STR_SIZE];
    Comp_comm_group_mgt_node *comp_node = comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id, false, "H2D_remapping_wgt_file_info::read_grid_size");


    IO_netcdf *netcdf_file_object = new IO_netcdf("remapping weights file for H2D interpolation", wgt_file_name, "r", false);
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, grid_size = netcdf_file_object->get_dimension_size(label, comp_node->get_comm_group(), comp_node->get_current_proc_local_id() == 0));
    delete netcdf_file_object;
}
...
void H2D_remapping_wgt_file_info::read_weight_grid_data(int comp_id, const char *label, const char *required_data_type, void **buffer_ptr, int buffer_size, bool necessary)
{
    int field_size, i;
    char data_type[NAME_STR_SIZE];
    Comp_comm_group_mgt_node *comp_node = comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id, false, "in H2D_remapping_wgt_file_info::read_weight_grid_data");

    IO_netcdf *netcdf_file_object = new IO_netcdf("remapping weights file for H2D interpolation", wgt_file_name, "r", false);
    netcdf_file_object->read_file_field(label, buffer_ptr, &field_size, data_type, comp_node->get_comm_group(), comp_node->get_current_proc_local_id() == 0);
	if (necessary)
...
H2D_remapping_wgt_file_info *H2D_remapping_wgt_file_mgt::search_H2D_remapping_weight(Original_grid_info *src_original_grid, Original_grid_info *dst_original_grid, const char *algorithm_name, int comp_id)
{
    ...
    for (int i = 0; i < H2D_remapping_wgt_files.size(); i ++)
        if (H2D_remapping_wgt_files[i]->match_H2D_remapping_wgt(src_original_grid, dst_original_grid)) {
			EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "the specified remapping weight file \"%s\" is used", H2D_remapping_wgt_files[i]->get_wgt_file_name());
	...
	if (src_original_grid->get_H2D_sub_CoR_grid() != NULL && algorithm_name != NULL) {	
		...
		if (existing_wgt_file != NULL) {
			EXECUTION_REPORT_LOG(REPORT_LOG, comp_id, true, "Default remapping weight file \"%s\" has already been used before", full_default_wgt_file_name);
		...
		else EXECUTION_REPORT_LOG(REPORT_LOG, comp_id, true, "Default remapping weight file \"%s\" does not exist", full_default_wgt_file_name);
	}
...
Remapping_algorithm_specification::Remapping_algorithm_specification(const Remapping_algorithm_specification *src_specification)
{
	...
	this->comp_id = src_specification->comp_id;
...
Remapping_algorithm_specification::Remapping_algorithm_specification(int comp_id, TiXmlElement *XML_element, const char *XML_file_name, int algorithm_type)
{
    ...
    else EXECUTION_REPORT(REPORT_ERROR, -1, false, "Software error in Remapping_algorithm_specification::Remapping_algorithm_specification");
...
long Remapping_setting::calculate_checksum()
{
    ...
    long checksum = calculate_checksum_of_array(temp_array, buffer_content_size, 1, NULL, MPI_COMM_NULL);
    delete [] temp_array;
