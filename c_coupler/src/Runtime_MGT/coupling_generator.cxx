...
MPI_Comm create_union_comm_common(MPI_Comm comm1, MPI_Comm comm2, int current_proc_id1, int current_proc_id2, std::vector<int> &procs_global_ids1, std::vector<int> &procs_global_ids2, int connection_id, int *proc_ranks_in_union_comm1, int *proc_ranks_in_union_comm2)
{
    ...
    if (current_proc_id1 != -1 && current_proc_id2 != -1 && comm1 == comm2) {
        for (int i = 0; i < comp_num_procs1; i ++) {
            proc_ranks_in_union_comm1[i] = i;
            proc_ranks_in_union_comm2[i] = i;
        }
        return comm1;
    }

    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "create_union_comm");
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Comm_group(MPI_COMM_WORLD, &common_group) == MPI_SUCCESS);
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_incl(common_group, comp_num_procs1, proc_ranks1, &comm_group1) == MPI_SUCCESS);
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_incl(common_group, comp_num_procs2, proc_ranks2, &comm_group2) == MPI_SUCCESS);
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_intersection(comm_group1, comm_group2, &intersection_group) == MPI_SUCCESS);
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_size(intersection_group, &intersection_size) == MPI_SUCCESS);

    if (current_proc_id1 != -1)
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Barrier(comm1) == MPI_SUCCESS);
    if (current_proc_id2 != -1)
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Barrier(comm2) == MPI_SUCCESS);
    ...
    if (intersection_size == comp_num_procs1)
        ...
        if (current_proc_id1 != -1) {
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_create(comm1, 0, MPI_COMM_WORLD, procs_global_ids2[0], connection_id, &inter_comm) == MPI_SUCCESS);
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_merge(inter_comm, true, &union_comm) == MPI_SUCCESS);
        }
        else if (current_proc_id2 != -1) {
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_create(comm2, 0, MPI_COMM_WORLD, procs_global_ids1[0], connection_id, &inter_comm) == MPI_SUCCESS);
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_merge(inter_comm, true, &union_comm) == MPI_SUCCESS);
        }
    }
    else {
        ...
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_translate_ranks(comm_group2, comp_num_procs2, proc_ranks2, comm_group1, translate_ranks) == MPI_SUCCESS);
        if (current_proc_id2 != -1) {
            int color = 1;
            if (current_proc_id1 != -1)
                color = 0;
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Comm_split(comm2, color, 0, &exclusive_comm) == MPI_SUCCESS);
        }
        if (current_proc_id1 != -1) {
            ...
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_create(comm1, 0, MPI_COMM_WORLD, procs_global_ids2[root_indx_in_exclusive_comm], connection_id, &inter_comm) == MPI_SUCCESS);
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_merge(inter_comm, true, &union_comm) == MPI_SUCCESS);
        }
        else {
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_create(exclusive_comm, 0, MPI_COMM_WORLD, procs_global_ids1[0], connection_id, &inter_comm) == MPI_SUCCESS);
            EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Intercomm_merge(inter_comm, true, &union_comm) == MPI_SUCCESS);
        }
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Comm_group(union_comm, &union_group) == MPI_SUCCESS);
    if (proc_ranks_in_union_comm1 != NULL)
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_translate_ranks(comm_group1, comp_num_procs1, proc_ranks1, union_group, proc_ranks_in_union_comm1) == MPI_SUCCESS);
    if (proc_ranks_in_union_comm2 != NULL)
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_translate_ranks(comm_group2, comp_num_procs2, proc_ranks2, union_group, proc_ranks_in_union_comm2) == MPI_SUCCESS);

    delete [] proc_ranks1;
    delete [] proc_ranks2;

    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "create_union_comm");

    return union_comm;
}


void create_union_comm_for_two_comps(Comp_comm_group_mgt_node *src_comp_node, Comp_comm_group_mgt_node *dst_comp_node, MPI_Comm &union_comm, int **src_proc_ranks_in_union_comm, int **dst_proc_ranks_in_union_comm, int connection_id)
{
    ...
    if (current_proc_id_src_comp != -1) {
        EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "start to create union comm between components \"%s\" and \"%s\". The connection id is %d", src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), connection_id);
        MPI_Barrier(src_comp_node->get_comm_group());
    }
    if (current_proc_id_dst_comp != -1) {
        EXECUTION_REPORT_LOG(REPORT_LOG, dst_comp_node->get_comp_id(), true, "start to create union comm between components \"%s\" and \"%s\". The connection id is %d", src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), connection_id);
    ...
    comps_full_names.push_back(src_comp_node->get_full_name());
    if (dst_comp_node != src_comp_node)
        comps_full_names.push_back(dst_comp_node->get_full_name());
    Union_comm_info *union_comm_info = union_comm_mgr->search_union_comm(comps_full_names);
    if (union_comm_info == NULL) {
        ...
        union_comm_info = union_comm_mgr->add_union_comm(comps_full_names, union_comm);
        union_comm_info->set_proc_ranks_in_union_comm(src_comp_node->get_comp_full_name(), *src_proc_ranks_in_union_comm, src_comp_node->get_num_procs());
        if (dst_comp_node != src_comp_node)
            union_comm_info->set_proc_ranks_in_union_comm(dst_comp_node->get_comp_full_name(), *dst_proc_ranks_in_union_comm, dst_comp_node->get_num_procs());
    }
    else {
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "existing union comm1 for %s to %s", src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name());
        union_comm = union_comm_info->get_union_comm();
        union_comm_info->get_proc_ranks_in_union_comm(src_comp_node->get_comp_full_name(), *src_proc_ranks_in_union_comm, src_comp_node->get_num_procs());
        union_comm_info->get_proc_ranks_in_union_comm(dst_comp_node->get_comp_full_name(), *dst_proc_ranks_in_union_comm, dst_comp_node->get_num_procs());
    }

    if (current_proc_id_src_comp != -1)
        EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "Finish creating union comm between components \"%s\" and \"%s\". The connection id is %d", src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), connection_id);
    if (current_proc_id_dst_comp != -1)
        EXECUTION_REPORT_LOG(REPORT_LOG, dst_comp_node->get_comp_id(), true, "Finish creating union comm between components \"%s\" and \"%s\". The connection id is %d", src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), connection_id);
}
...
Coupling_connection::~Coupling_connection()
{
    ...
    if (src_proc_ranks_in_union_comm != NULL)
        delete src_proc_ranks_in_union_comm;
    if (dst_proc_ranks_in_union_comm != NULL)
        delete dst_proc_ranks_in_union_comm;
    for (int i = 0; i < src_fields_info.size(); i ++)
        delete src_fields_info[i];
    for (int i = 0; i < dst_fields_info.size(); i ++)
        delete dst_fields_info[i];
}
...
void Coupling_connection::generate_a_coupling_procedure(bool has_frac_remapping)
{
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "generate_interpolation");
    generate_interpolation(has_frac_remapping);
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "generate_interpolation");
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "generate_data_transfer");
    generate_data_transfer();
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "generate_data_transfer");
...
{
    create_union_comm_for_two_comps(src_comp_node, dst_comp_node, union_comm, &src_proc_ranks_in_union_comm, &dst_proc_ranks_in_union_comm, connection_id);
}
...
void Coupling_connection::generate_data_transfer()
{
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "MPI_Win_create1");
    if (current_proc_id_dst_comp != -1) {
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "MPI_Win_create1");
#endif
...
bool Coupling_connection::exchange_grid(Comp_comm_group_mgt_node *sender_comp_node, Comp_comm_group_mgt_node *receiver_comp_node, const char *grid_name)
{
    ...
    Original_grid_info *sender_H2D_sub_grid = NULL, *receiver_H2D_sub_grid = NULL;

    Original_grid_info *sender_original_grid = original_grid_mgr->search_grid_info(grid_name, sender_comp_node->get_comp_id());
    Original_grid_info *receiver_original_grid = NULL;

    if (sender_comp_node == receiver_comp_node)
        return false;

    original_grid_status = 0;
    if (receiver_comp_node->get_current_proc_local_id() != -1)
        original_grid_status = sender_original_grid != NULL ? 1 : 0;
    EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Allreduce(&original_grid_status, &all_original_grid_status, 1, MPI_INT, MPI_SUM, union_comm) == MPI_SUCCESS);
    if (all_original_grid_status == receiver_comp_node->get_num_procs()) {
        ...
        receiver_original_grid = sender_original_grid;
    }
    else {
        if (sender_comp_node->get_current_proc_local_id() != -1)
        ...
        if (src_comp_node->get_current_proc_local_id() != -1)
        ...
        if (sender_original_grid == NULL) {
            read_data_from_array_buffer(&checksum_lat, sizeof(long), temp_array_buffer, buffer_content_size, true);
            read_data_from_array_buffer(&checksum_lon, sizeof(long), temp_array_buffer, buffer_content_size, true);
            ...
            Remap_grid_class *mirror_grid = new Remap_grid_class(NULL, sender_comp_node->get_full_name(), temp_array_buffer, buffer_content_size, true);
            ...
            receiver_original_grid->set_grid_checksum(checksum_mask, checksum_lon, checksum_lat);
            receiver_original_grid->link_sub_grids();
            if (receiver_original_grid->get_H2D_sub_CoR_grid() != NULL) {
                receiver_H2D_sub_grid = original_grid_mgr->search_grid_info(receiver_original_grid->get_comp_id(), receiver_original_grid->get_H2D_sub_CoR_grid());
                if (receiver_H2D_sub_grid == NULL)
                    receiver_H2D_sub_grid = original_grid_mgr->get_original_grid(original_grid_mgr->add_original_grid(receiver_original_grid->get_comp_id(), receiver_original_grid->get_H2D_sub_CoR_grid()->get_grid_name(), receiver_original_grid->get_H2D_sub_CoR_grid()));
                receiver_H2D_sub_grid->set_grid_checksum(receiver_original_grid->get_checksum_H2D_mask(), receiver_original_grid->get_checksum_H2D_center_lon(), receiver_original_grid->get_checksum_H2D_center_lat());
            }
        }
        else receiver_original_grid = sender_original_grid;
    }
    ...
    if (receiver_original_grid->get_H2D_sub_CoR_grid() == NULL)
        return all_original_grid_status != receiver_comp_node->get_num_procs();

    return true;
}


void Coupling_connection::exchange_distributed_H2D_grid(Comp_comm_group_mgt_node *sender_comp_node, Comp_comm_group_mgt_node *receiver_comp_node, Original_grid_info *sender_original_grid, bool bypass)
{
    int *original_grid_status = NULL, all_original_grid_status;
    Original_grid_info *receiver_H2D_sub_grid = NULL, *sender_H2D_sub_grid = NULL;
    long buffer_content_size = sizeof(int);


    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, sender_original_grid != NULL, "Software error in Coupling_connection::exchange_distributed_H2D_grid");

    if (sender_comp_node == receiver_comp_node || sender_original_grid->get_H2D_sub_CoR_grid() == NULL)
        return;

    original_grid_status = new int [1];
    original_grid_status[0] = 0;
    if (receiver_comp_node->get_current_proc_local_id() != -1 != NULL) {
        receiver_H2D_sub_grid = original_grid_mgr->search_grid_info(sender_original_grid->get_comp_id(), sender_original_grid->get_H2D_sub_CoR_grid());
        if (receiver_H2D_sub_grid == NULL)
            receiver_H2D_sub_grid = original_grid_mgr->get_original_grid(original_grid_mgr->add_original_grid(sender_original_grid->get_comp_id(), sender_original_grid->get_H2D_sub_CoR_grid()->get_grid_name(), sender_original_grid->get_H2D_sub_CoR_grid()));
        original_grid_status[0] = (distributed_H2D_grid_mgr->search_distributed_H2D_grid(receiver_comp_node->get_full_name(), receiver_H2D_sub_grid) != NULL || bypass) ? 1 : 0;
        receiver_H2D_sub_grid->set_grid_checksum(sender_original_grid->get_checksum_H2D_mask(), sender_original_grid->get_checksum_H2D_center_lon(), sender_original_grid->get_checksum_H2D_center_lat());
    }
    if (report_error_enabled) {
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Allreduce(original_grid_status, &all_original_grid_status, 1, MPI_INT, MPI_SUM, union_comm) == MPI_SUCCESS);
        EXECUTION_REPORT(REPORT_ERROR, -1, all_original_grid_status == receiver_comp_node->get_num_procs() || all_original_grid_status == 0, "Software error in Coupling_connection::exchange_grid");
    }
    transfer_array_from_one_comp_to_another(receiver_comp_node->get_current_proc_local_id(), receiver_comp_node->get_root_proc_global_id(), sender_comp_node->get_current_proc_local_id(), sender_comp_node->get_root_proc_global_id(), sender_comp_node->get_comm_group(), (char**)(&original_grid_status), buffer_content_size);
    if (original_grid_status[0] == 1) {
        delete [] original_grid_status;
        return;
    }

    delete [] original_grid_status;
    if (sender_comp_node->get_current_proc_local_id() != -1) {
        sender_H2D_sub_grid = original_grid_mgr->search_grid_info(sender_comp_node->get_comp_id(), sender_original_grid->get_H2D_sub_CoR_grid());
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, sender_H2D_sub_grid != NULL, "Software error in Coupling_connection::exchange_grid");
    }

    original_grid_mgr->generate_distributed_H2D_grid_between_comps(sender_comp_node->get_comp_full_name(), sender_H2D_sub_grid, receiver_comp_node->get_full_name(), receiver_H2D_sub_grid);
}
...
void Coupling_connection::add_bottom_field_coupling_info(int field_connection_indx, Runtime_remapping_weights_container *remapping_weights_container, Remapping_setting *remapping_setting, H2D_remapping_wgt_file_info *H2D_remapping_weight_file)
{
    ...
    if (V3D_remapping_weights->get_parallel_remapping_weights() != NULL) {
        ...
        else if (bottom_field_coupling_info->bottom_field_inst->get_field_data()->get_coord_value_grid() != NULL)
            dynamic_V1D_remap_weight_of_operator->get_field_data_grid_src()->set_level_V3D_coord_dynamic_trigger_field(bottom_field_coupling_info->bottom_field_inst->get_field_data());
        ...
        if (V3D_remapping_weights->get_dst_original_grid()->get_bottom_field_variation_type() == BOTTOM_FIELD_VARIATION_EXTERNAL) {
            ...
            if (dynamic_V1D_remap_weight_of_operator->get_field_data_grid_dst()->get_level_V3D_coord_dynamic_trigger_field() != NULL)
                ...
                dynamic_V1D_remap_weight_of_operator->get_field_data_grid_dst()->set_level_V3D_coord_dynamic_trigger_field(bottom_field_coupling_info->bottom_field_inst->get_field_data());
        }
    }
...
void Coupling_connection::generate_interpolation(bool has_frac_remapping)
{
    ...
    for (int i = 0; i < fields_name.size(); i ++) {
        EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "src_fields_info[i]->grid_name \"%s\" and dst_fields_info[i]->grid_name \"%s\". ", src_fields_info[i]->grid_name, dst_fields_info[i]->grid_name);
    }
    for (int i = 0; i < fields_name.size(); i ++) {
        EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "fields_name.size() \"%d\" and fields_name \"%s\". ", fields_name.size(), fields_name[i]);
        ...
        H2D_remapping_weight_file = NULL;
...
//        exchange_grid(dst_comp_node, src_comp_node, dst_fields_info[i]->grid_name);
        comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "exchange_grid");
        exchange_grid(src_comp_node, dst_comp_node, src_fields_info[i]->grid_name);
        comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "exchange_grid");
        if (current_proc_id_dst_comp != -1) {
            dst_original_grid = original_grid_mgr->search_grid_info(dst_fields_info[i]->grid_name, comp_comm_group_mgt_mgr->search_global_node(dst_comp_full_name)->get_comp_id());
            src_original_grid = original_grid_mgr->search_grid_info(src_fields_info[i]->grid_name, comp_comm_group_mgt_mgr->search_global_node(src_comp_interfaces[0].first)->get_comp_id());
            grids_are_the_same = false;
            if (src_original_grid->is_the_same_as_another_grid(dst_comp_node->get_comp_full_name(), dst_original_grid)) {
            ...
            if (!grids_are_the_same) {
                ...
                if (src_original_grid->get_H2D_sub_CoR_grid() != NULL)
                    H2D_remapping_weight_file = (&field_remapping_setting)->search_H2D_remapping_weight(src_original_grid->get_max_sub_grid_under_V3D(), dst_original_grid->get_max_sub_grid_under_V3D(), comp_comm_group_mgt_mgr->search_global_node(dst_comp_full_name)->get_comp_id());
            }
        }

        comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "exchange_distributed_H2D_grid");
        src_original_grid = original_grid_mgr->search_grid_info(src_fields_info[i]->grid_name, src_comp_node->get_comp_id());
        exchange_distributed_H2D_grid(src_comp_node, dst_comp_node, src_original_grid, H2D_remapping_weight_file != NULL || grids_are_the_same);
        comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "exchange_distributed_H2D_grid");
        if (grids_are_the_same)
            continue;

        if (current_proc_id_dst_comp != -1) {
            dst_fields_info[i]->runtime_remapping_weights_container = new Runtime_remapping_weights_container(src_comp_node->get_comp_full_name(), dst_comp_node->get_comp_full_name(), src_original_grid, dst_original_grid, &field_remapping_setting, decomps_info_mgr->search_decomp_info(dst_fields_info[i]->decomp_name, dst_comp_node->get_comp_id()), H2D_remapping_weight_file);
            ...
            if (src_original_grid->has_sigma_sub_grid() || src_original_grid->does_use_V3D_level_coord())
                add_bottom_field_coupling_info(i, dst_fields_info[i]->runtime_remapping_weights_container, &field_remapping_setting, H2D_remapping_weight_file);
        }
    }
...
Union_comm_info::Union_comm_info(std::vector<const char *> &comps_full_names, MPI_Comm union_comm)
{
    for (int i = 0; i < comps_full_names.size(); i ++) {
        this->comps_full_names.push_back(strdup(comps_full_names[i]));
        proc_ranks_in_union_comm.push_back(NULL);
    }

    ((Coupling_generator*) (NULL))->sort_comp_full_names(this->comps_full_names, NULL);
    this->union_comm = union_comm;
}


void Union_comm_info::set_proc_ranks_in_union_comm(const char *comp_full_name, int *proc_ranks, int size)
{
    int pos;

    for (pos = 0; pos < comps_full_names.size(); pos ++)
        if (words_are_the_same(comp_full_name, comps_full_names[pos]))
            break;

    EXECUTION_REPORT(REPORT_ERROR, -1, pos < comps_full_names.size() && proc_ranks_in_union_comm[pos] == NULL, "Software error in Union_comm_info::set_proc_ranks_in_union_comm");
    proc_ranks_in_union_comm[pos] = new int [size];
    for (int i = 0; i < size; i ++)
        (proc_ranks_in_union_comm[pos])[i] = proc_ranks[i];
}


void Union_comm_info::get_proc_ranks_in_union_comm(const char *comp_full_name, int *proc_ranks, int size)
{
    int pos;
    Comp_comm_group_mgt_node *comp_node = comp_comm_group_mgt_mgr->search_global_node(comp_full_name);


    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, comp_node != NULL && comp_node->get_num_procs() == size, "Software error in Union_comm_info::get_proc_ranks_in_union_comm");

    for (pos = 0; pos < comps_full_names.size(); pos ++)
        if (words_are_the_same(comp_full_name, comps_full_names[pos]))
            break;

    EXECUTION_REPORT(REPORT_ERROR, -1, pos < comps_full_names.size(), "Software error in Union_comm_info::set_proc_ranks_in_union_comm");

    if (proc_ranks_in_union_comm[pos] == NULL) {
        MPI_Group common_group, union_group, comp_group;
        proc_ranks_in_union_comm[pos] = new int [size];
        int *proc_ranks1 = new int [size];
        for (int i = 0; i < size; i ++) {
            proc_ranks_in_union_comm[pos][i] = comp_node->get_local_proc_global_id(i);
            proc_ranks1[i] = i;
        }
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Comm_group(MPI_COMM_WORLD, &common_group) == MPI_SUCCESS);
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_incl(common_group, size, proc_ranks_in_union_comm[pos], &comp_group) == MPI_SUCCESS);
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Comm_group(union_comm, &union_group) == MPI_SUCCESS);
        EXECUTION_REPORT(REPORT_ERROR, -1, MPI_Group_translate_ranks(comp_group, size, proc_ranks1, union_group, proc_ranks_in_union_comm[pos]) == MPI_SUCCESS);
        delete [] proc_ranks1;
    }

    for (int i = 0; i < size; i ++)
        proc_ranks[i] = (proc_ranks_in_union_comm[pos])[i];
}


bool Union_comm_info::match(std::vector<const char *> &comps_full_names)
{
    int count = 0;

    if (this->comps_full_names.size() != comps_full_names.size())
        return false;

    for (int i = 0; i < comps_full_names.size(); i ++)
        for (int j = 0; j < comps_full_names.size(); j ++)
            if (words_are_the_same(this->comps_full_names[i], comps_full_names[j])) {
                count ++;
                break;
            }

    return count == comps_full_names.size();
}


Union_comm_info::~Union_comm_info()
{
    for (int i = 0; i < comps_full_names.size(); i ++) {
        delete [] comps_full_names[i];
        if (proc_ranks_in_union_comm[i] != NULL)
            delete [] proc_ranks_in_union_comm[i];
    }
}


Union_comm_info *Union_comm_mgt::search_union_comm(std::vector<const char *> &comps_full_names)
{
    int i;

    for (i = 0; i < union_comms.size(); i ++)
        if (union_comms[i]->match(comps_full_names))
            return union_comms[i];

    return NULL;
}


Union_comm_info *Union_comm_mgt::add_union_comm(std::vector<const char *> &comps_full_names, MPI_Comm union_comm)
{
    int i;

    if (report_error_enabled) {
        for (i = 0; i < union_comms.size(); i ++)
            EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, !union_comms[i]->match(comps_full_names), "Software error in Union_comm_mgt::add_union_comm");
    }

    union_comms.push_back(new Union_comm_info(comps_full_names, union_comm));
    return union_comms[union_comms.size() - 1];
}


Union_comm_mgt::~Union_comm_mgt()
{
    for (int i = 0; i < union_comms.size(); i ++)
        delete union_comms[i];
}
...
void Coupling_generator::generate_coupling_procedures_common(int API_id, MPI_Comm comm, bool is_overall_generation, bool is_internal_generation, int comp_id_ignore_full_name, const char *annotation)
{
    ...
    Comp_comm_group_mgt_node *local_comp_node = NULL, *existing_comp_node;
    ...
    for (int i = 0; i < all_comp_fullnames_for_coupling_generation.size(); i ++) {
        existing_comp_node = comp_comm_group_mgt_mgr->load_comp_info_from_XML(local_comp_node->get_comp_id(), all_comp_fullnames_for_coupling_generation[i], comm);
...
void Coupling_generator::do_external_coupling_generation(int API_id, const char *annotation)
{
    ...
    std::vector<Comp_comm_group_mgt_node*> all_comp_nodes;
    Comp_comm_group_mgt_node *local_comp_node, *existing_comp_node;
    ...
    for (i = 0; i < all_comp_fullnames_for_coupling_generation.size(); i ++) {
        ...
        for (j = 0; j < all_comp_fullnames_for_coupling_generation.size(); j ++) {
            ...
            existing_comp_node = comp_comm_group_mgt_mgr->load_comp_info_from_XML(local_comp_node->get_comp_id(), all_comp_fullnames_for_coupling_generation[j], local_comp_node->get_comm_group());
            ...
            if (k == all_comp_nodes.size())
                all_comp_nodes.push_back(existing_comp_node);
        }
    }
    ...
    Union_comm_info *union_comm_info = union_comm_mgr->search_union_comm(all_comp_fullnames_for_coupling_generation);
    if (union_comm_info == NULL) {
        for (i = 0; i < all_comp_fullnames_for_coupling_generation.size(); i ++) {
            ...
            if (i == 0) {
                ...
                for (int j = 0; j < array_size / sizeof(int); j ++)
                    proc_global_ids_in_union_comm.push_back(temp_proc_global_ids[j]);
        ...
        union_comm_info = union_comm_mgr->add_union_comm(all_comp_fullnames_for_coupling_generation, union_comm);
    }
    else union_comm = union_comm_info->get_union_comm();
...
void Coupling_generator::generate_simple_data_transfer_connection(const char *src_host_comp_full_name, const char *src_comp_full_name, int num_src_fields, Field_mem_info **src_fields_mem, const char *dst_host_comp_full_name, const char *dst_comp_full_name, int num_dst_fields, Field_mem_info **dst_fields_mem, Runtime_trans_algorithm **runtime_send_algorithm, Runtime_trans_algorithm **runtime_recv_algorithm)
{
    int num_fields = num_src_fields > 0 ? num_src_fields : num_dst_fields;
    ...
    MPI_Comm union_comm;
    MPI_Win data_win, tag_win;
    char *src_decomp_name = new char [NAME_STR_SIZE], *dst_decomp_name = new char [NAME_STR_SIZE];
    long array_size = NAME_STR_SIZE;
    int connection_id;
    int *src_proc_ranks_in_union_comm = new int [src_comp_node->get_num_procs()], *dst_proc_ranks_in_union_comm = new int [dst_comp_node->get_num_procs()];

    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "generate_simple_data_transfer_connection");
    if (num_src_fields > 0)
        EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "Start to generate simple runtime data transfer algorithm from component \"%s\" (current component) to \"%s\" with %d fields.", src_host_comp_full_name, dst_host_comp_full_name, num_src_fields);
    if (num_dst_fields > 0)
        EXECUTION_REPORT_LOG(REPORT_LOG, dst_comp_node->get_comp_id(), true, "Start to generate simple runtime data transfer algorithm from component \"%s\" to \"%s\" (current component) with %d fields.", src_host_comp_full_name, dst_host_comp_full_name, num_dst_fields);

    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_src_fields > 0 && src_comp_node->get_current_proc_local_id() != -1 || num_src_fields == 0 && src_comp_node->get_current_proc_local_id() == -1, "Software error Coupling_generator::generate_simple_data_transfer_connection");
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_dst_fields > 0 && dst_comp_node->get_current_proc_local_id() != -1 || num_dst_fields == 0 && dst_comp_node->get_current_proc_local_id() == -1, "Software error Coupling_generator::generate_simple_data_transfer_connection");

    if (num_src_fields > 0 && num_dst_fields > 0)
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_src_fields == num_dst_fields, "Software error in Coupling_generator::generate_simple_data_transfer_connection: %d vs %d", num_src_fields, num_dst_fields);

    *runtime_send_algorithm = NULL;
    *runtime_recv_algorithm = NULL;

    create_union_comm_for_two_comps(src_comp_node, dst_comp_node, union_comm, &src_proc_ranks_in_union_comm, &dst_proc_ranks_in_union_comm, 100000);

    coupling_generator->synchronize_latest_connection_id(union_comm);
    connection_id = coupling_generator->apply_connection_id();

    for (int i = 1; i < num_src_fields; i ++)
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, src_fields_mem[i]->get_decomp_id() == src_fields_mem[0]->get_decomp_id());

    for (int i = 0; i < num_fields; i ++) {
        int src_decomp_comp_id, dst_decomp_comp_id;
        if (num_src_fields > 0)
            strcpy(src_decomp_name, src_fields_mem[i]->get_decomp_name());
        if (num_dst_fields > 0)
            strcpy(dst_decomp_name, dst_fields_mem[i]->get_decomp_name());
        transfer_array_from_one_comp_to_another(src_comp_node->get_current_proc_local_id(), src_comp_node->get_local_proc_global_id(0), dst_comp_node->get_current_proc_local_id(), dst_comp_node->get_local_proc_global_id(0), dst_comp_node->get_comm_group(), &src_decomp_name, array_size);
        transfer_array_from_one_comp_to_another(dst_comp_node->get_current_proc_local_id(), dst_comp_node->get_local_proc_global_id(0), src_comp_node->get_current_proc_local_id(), src_comp_node->get_local_proc_global_id(0), src_comp_node->get_comm_group(), &dst_decomp_name, array_size);
        if (num_dst_fields > 0)
            dst_decomp_comp_id = comp_comm_group_mgt_mgr->search_global_node(dst_comp_full_name)->get_comp_id();
        else dst_decomp_comp_id = dst_comp_node->get_comp_id();
        if (num_src_fields > 0)
            src_decomp_comp_id = comp_comm_group_mgt_mgr->search_global_node(src_comp_full_name)->get_comp_id();
        else src_decomp_comp_id = src_comp_node->get_comp_id();
        fields_router[i] = routing_info_mgr->search_or_add_router(src_decomp_comp_id, dst_decomp_comp_id, src_decomp_name, dst_decomp_name);
    }
    if (num_dst_fields > 0)
        *runtime_recv_algorithm = new Runtime_trans_algorithm(false, num_dst_fields, dst_fields_mem, fields_router, union_comm, src_proc_ranks_in_union_comm, connection_id);
    if (num_src_fields > 0)
        *runtime_send_algorithm = new Runtime_trans_algorithm(true, num_src_fields, src_fields_mem, fields_router, union_comm, dst_proc_ranks_in_union_comm, connection_id);

#ifdef USE_ONE_SIDED_MPI
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "MPI_Win_create2");
    if (num_dst_fields > 0) {
        MPI_Win_create((*runtime_recv_algorithm)->get_total_buf(), (*runtime_recv_algorithm)->get_total_buf_size()*sizeof(char), sizeof(char), MPI_INFO_NULL, union_comm, &data_win);
        (*runtime_recv_algorithm)->set_data_win(data_win);
        inout_interface_mgr->add_MPI_win(data_win);
    }
    if (num_src_fields > 0) {
        if (num_dst_fields == 0) {
        ...
        (*runtime_send_algorithm)->set_data_win(data_win);
    }
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "MPI_Win_create2");
#endif

    delete [] fields_router;
    delete [] src_decomp_name;
    delete [] dst_decomp_name;
    delete [] src_proc_ranks_in_union_comm;
    delete [] dst_proc_ranks_in_union_comm;

    if (num_src_fields > 0)
        EXECUTION_REPORT_LOG(REPORT_LOG, src_comp_node->get_comp_id(), true, "Finish generating simple runtime data transfer algorithm from component \"%s\" (current component) to \"%s\".", src_host_comp_full_name, dst_host_comp_full_name);
    if (num_dst_fields > 0)
        EXECUTION_REPORT_LOG(REPORT_LOG, dst_comp_node->get_comp_id(), true, "Finish generating simple runtime data transfer algorithm from component \"%s\" to \"%s\" (current component).", src_host_comp_full_name, dst_host_comp_full_name);
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "generate_simple_data_transfer_connection");
}
