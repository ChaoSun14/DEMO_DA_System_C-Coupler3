    ...
    for (int i = 0; i < coupling_connection->src_fields_info.size(); i ++) {
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT && !(inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)) {
            if (fields_time_info_dst->inst_or_aver == USING_AVERAGE_VALUE)
                fields_mem_inner_step_averaged[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_AVERAGED_INNER, coupling_connection->connection_id, NULL, false, false);
            ...
            if (fields_time_info_dst->inst_or_aver == USING_AVERAGE_VALUE && !(fields_time_info_dst->lag_seconds == 0 && fields_time_info_dst->timer->is_the_same_with(fields_time_info_src->timer)))
                fields_mem_inter_step_averaged[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_AVERAGED_INTER, coupling_connection->connection_id, NULL, false, false);
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT && !(inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)) {
            if (!words_are_the_same(transfer_data_type, coupling_connection->src_fields_info[i]->data_type)) {
                ...
                fields_mem_datatype_transformed[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_DATATYPE_TRANS, coupling_connection->connection_id, transfer_data_type,
                                                     inout_interface->get_interface_source() == INTERFACE_SOURCE_REGISTER && i < coupling_connection->fields_name.size(), false);
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_IMPORT && !(inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE)) {
            ...
            if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container == NULL || coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container->is_empty()) {
                ...
                if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container == NULL && words_are_the_same(coupling_connection->get_src_comp_full_name(), coupling_connection->get_dst_comp_full_name()) && !words_are_the_same(coupling_connection->dst_fields_info[i]->grid_name, coupling_connection->src_fields_info[i]->grid_name)) {
                    ...
                    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, src_original_grid != NULL && (src_original_grid->get_original_CoR_grid()->is_similar_grid_with(dst_original_grid->get_original_CoR_grid()) || src_original_grid->is_the_same_as_another_grid(coupling_connection->get_dst_comp_full_name(), dst_original_grid)), "Software error");
                ...
                if (src_original_grid != NULL) {
                    ...
                    fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), fields_mem_registered[i]->get_decomp_id(),
                                             ...
                                             transfer_data_type, fields_mem_registered[i]->get_unit(), "internal", inout_interface->get_interface_source() == INTERFACE_SOURCE_REGISTER && i < coupling_connection->fields_name.size(), false);
                }
                else fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_DATA_TRANSFER, coupling_connection->connection_id, transfer_data_type, false, false);
            }
            else {
                if (coupling_connection->dst_fields_info[i]->runtime_remapping_weights_under_V3D != NULL)
                    fields_mem_transfer[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), coupling_connection->dst_fields_info[i]->runtime_remapping_weights_under_V3D->get_src_decomp_info()->get_decomp_id(),
                                             coupling_connection->dst_fields_info[i]->runtime_remapping_weights_container->get_src_original_grid()->get_grid_id(), BUF_MARK_DATA_TRANSFER ^ coupling_connection->connection_id, transfer_data_type, fields_mem_registered[i]->get_unit(), "internal", false, false);
                ...
                fields_mem_remapped[i] = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), fields_mem_registered[i]->get_decomp_id(), fields_mem_registered[i]->get_grid_id(),
                                         BUF_MARK_REMAP_NORMAL ^ coupling_connection->connection_id, transfer_data_type, fields_mem_registered[i]->get_unit(), "internal", false, false);
            ...
            if (!words_are_the_same(transfer_data_type, coupling_connection->dst_fields_info[i]->data_type)) {
                ...
                fields_mem_datatype_transformed[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_DATATYPE_TRANS, coupling_connection->connection_id, coupling_connection->dst_fields_info[i]->data_type, false, false);
        ...
        if (inout_interface->get_interface_type() == COUPLING_INTERFACE_MARK_EXPORT) {
            ...
            if (last_field_instance != NULL && last_field_instance == fields_mem_transfer[i])
                EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, last_field_instance->get_comp_id() == fields_mem_registered[i]->get_comp_id() && last_field_instance->get_grid_id() == fields_mem_registered[i]->get_grid_id() && last_field_instance->get_decomp_id() == fields_mem_registered[i]->get_decomp_id(), "Software error in Connection_coupling_procedure::Connection_coupling_procedure");
            if (inout_interface->get_parent_interface() != NULL && inout_interface->get_parent_interface()->get_interface_type() == COUPLING_INTERFACE_MARK_HALO_EXCHANGE) {
                EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, last_field_instance == NULL, "Software error in Connection_coupling_procedure::Connection_coupling_procedure");
            }
            else EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, last_field_instance != NULL, "Software error in Connection_coupling_procedure::Connection_coupling_procedure");
            if (last_field_instance == NULL || last_field_instance == fields_mem_transfer[i]) {
                if (fields_mem_transfer[i] != NULL)
                    memory_manager->delete_field_inst(fields_mem_transfer[i]);
...
void Connection_coupling_procedure::do_halo_exchange()
{
    ...
    current_remote_fields_time = -1;
    runtime_data_transfer_algorithm->set_rearranging_intra_a_component();
    runtime_data_transfer_algorithm->pass_transfer_parameters(current_remote_fields_time, inout_interface->get_bypass_counter());
    while (!runtime_data_transfer_algorithm->run(true));
}
...
Inout_interface::Inout_interface(const char *interface_name, int interface_id, int num_field_instances, int *field_instance_IDs, int *halo_region_IDs, int size_field_instance_IDs, int size_halo_region_IDs, const char *annotation)
{
    ...
    for (int i = 0; i < num_field_instances; i ++) {
        field_instance_halo = memory_manager->alloc_mem(fields_mem_registered[i]->get_field_name(), halo_region_IDs[i], fields_mem_registered[i]->get_grid_id(), BUF_MARK_HALO_EXCHANGE, fields_mem_registered[i]->get_data_type(), fields_mem_registered[i]->get_unit(), "C-Coupler internal", false, false);
...
void Inout_interface::common_checking_for_interface_registration(int num_fields, int *field_ids, int array_size, int timer_id, int inst_or_aver, int interface_type, const char *interface_name, int API_id, int interface_source, const char *field_ids_parameter_name, const char *annotation)
{
    ...
    for (int i = 0; i < num_fields; i ++) {
        EXECUTION_REPORT(REPORT_ERROR, -1, memory_manager->check_is_legal_field_instance_id(field_ids[i]), "Error happens when calling the API \"%s\" to register an interface named \"%s\": the parameter \"%s\" contains wrong field instance ID (the No. %d element (%s) of the array is wrong or the registration for the corresponding chunk field instance has not been finalized). Please verify the model code related to the annotation \"%s\"", API_label, interface_name, field_ids_parameter_name, i + 1, memory_manager->get_field_instance(field_ids[i])->get_field_name(), annotation);
        if (i == 0)
    ...
    if (interface_source != INTERFACE_SOURCE_IO_OUTPUT && (interface_type == COUPLING_INTERFACE_MARK_EXPORT || interface_type == COUPLING_INTERFACE_MARK_IMPORT))
        for (int i = 0; i < num_fields; i ++)
...
void Inout_interface::add_coupling_procedure(Connection_coupling_procedure *coupling_procedure)
{
    ...
    for (int i = fields_connected_status.size(); i < fields_mem_registered.size(); i ++)
        fields_connected_status.push_back(false);
    for (int i = fields_coupling_procedures.size(); i < fields_mem_registered.size(); i ++)
        fields_coupling_procedures.push_back(NULL);
...
void Inout_interface::execute(bool bypass_timer, int API_id, int *field_update_status, int size_field_update_status, const char *annotation)
{
    ...
    if (bypass_timer)
        bypass_counter ++;
    if (time_mgr->check_is_model_run_finished() && parent_interface == NULL && interface_source != INTERFACE_SOURCE_IO_OUTPUT) {
...
void Inout_interface::do_halo_exchange(int API_id, bool is_asynchronous, const char *annotation)
{
    ...
    if (!is_asynchronous) {
        ...
        children_interfaces[0]->get_unique_data_send_algorithm()->wait_sending_data();
    }
...
void Inout_interface::finish_halo_exchange(int API_id, const char *annotation)
{
    ...
    for (int i = 0; i < children_interfaces[1]->fields_mem_registered.size(); i ++)
        children_interfaces[1]->fields_mem_registered[i]->check_field_sum(report_internal_log_enabled, true, "after halo exchanging");
}
...
void Inout_interface::add_remappling_fraction_processing(void *frac_src, void *frac_dst, int size_frac_src, int size_frac_dst, const char *frac_data_type, int API_id, const char *annotation)
{
    ...
    Field_mem_info *frac_field_src = memory_manager->alloc_mem("remap_frac", template_field_src->get_decomp_id(), template_field_src->get_comp_or_grid_id(), BUF_MARK_REMAP_FRAC ^ coupling_generator->get_latest_connection_id(), frac_data_type, "unitless", "source fraction for remapping", false, false);
    frac_field_src->reset_mem_buf(frac_src, true, -1);
    Field_mem_info *frac_field_dst = memory_manager->alloc_mem("remap_frac", template_field_dst->get_decomp_id(), template_field_dst->get_comp_or_grid_id(), BUF_MARK_REMAP_FRAC ^ coupling_generator->get_latest_connection_id(), frac_data_type, "unitless", "target fraction for remapping", false, false);
    if (size_frac_dst != -1)
    ...
    for (int i = 0; i < children_interfaces[0]->fields_mem_registered.size(); i ++) {
        ...
        children_interfaces[0]->fields_mem_registered[i] = memory_manager->alloc_mem(fields_mem_registered[i], BUF_MARK_REMAP_FRAC, coupling_generator->get_latest_connection_id(), fields_mem_registered[i]->get_data_type(), false, false);
    }
...
Field_mem_info *Inout_interface::get_field_mem(int index)
{
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, index >= 0 && index < fields_mem_registered.size(), "Software error in Inout_interface::get_field_mem");
    return fields_mem_registered[index];
}


void Inout_interface::set_rearranging_intra_a_component()
{
    if (interface_type != COUPLING_INTERFACE_MARK_EXPORT)
        return;

    for (int i = 0; i < coupling_procedures.size(); i ++) {
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, coupling_procedures[i]->runtime_data_transfer_algorithm != NULL, "Software error in Inout_interface::set_rearranging_intra_a_component");
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "liuli set_rearranging_intra_a_component for %lx", coupling_procedures[i]->runtime_data_transfer_algorithm);
        coupling_procedures[i]->runtime_data_transfer_algorithm->set_rearranging_intra_a_component();
    }
}


Runtime_trans_algorithm *Inout_interface::get_unique_data_send_algorithm()
{
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, interface_type == COUPLING_INTERFACE_MARK_EXPORT, "Software error in Inout_interface::get_unique_data_send_algorithm");
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, coupling_procedures.size() == 1, "Software error in Inout_interface::get_unique_data_send_algorithm");
    EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, coupling_procedures[0]->runtime_data_transfer_algorithm != NULL, "Software error in Inout_interface::get_unique_data_send_algorithm");

    return coupling_procedures[0]->runtime_data_transfer_algorithm;
}
...
{
    Comp_comm_group_mgt_node *comp1_node = NULL, *comp2_node = NULL;
    ...
    if (comp1_node != comp2_node)
        comp2_node = comp_comm_group_mgt_mgr->load_comp_info_from_XML(comp1_node->get_comp_id(), comp2_full_name, comp1_node->get_comm_group());
    ...
    coupling_generator->synchronize_latest_connection_id(comp1_node->get_comm_group());
    Coupling_connection *coupling_connection = new Coupling_connection(coupling_generator->apply_connection_id());
    ...
    interface1->set_rearranging_intra_a_component();
    if (interface2 != NULL)
        interface2->set_rearranging_intra_a_component();
