...
void Performance_timing_unit::check_timing_unit(int unit_type, int unit_behavior, int unit_int_keyword, const char *unit_char_keyword)
{
    ...
    if (unit_type == TIMING_TYPE_IO)
        EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, unit_behavior == TIMING_IO_INPUT || unit_behavior == TIMING_IO_OUTPUT || unit_behavior == TIMING_IO_RESTART || unit_behavior == TIMING_OUTPUT_ALL,
                         "C-Coupler error in checking unit_behavior in match_timing_unit");
...
void Performance_timing_unit::timing_output()
{
    if (unit_type == TIMING_TYPE_IO) {
        ...
        else if (unit_behavior == TIMING_OUTPUT_ALL) 
            EXECUTION_REPORT(REPORT_CONSTANTLY, comp_id, true, "TIMING RESULT: the component model \"%s\" spends %lf seconds for preparing and writing output data at the current process\n", comp_comm_group_mgt_mgr->get_global_node_of_local_comp(comp_id,false,"")->get_full_name(), total_time);
