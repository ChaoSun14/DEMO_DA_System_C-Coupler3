...
//        EXECUTION_REPORT(REPORT_ERROR, parent->get_comp_id(), false, "Error happens when registering the component model \"%s\": its parent \"%s\" is an inactive component model and already has one child \"%s\". Please note that an inactive component model can have at most one child. Please check the model code with the annotation \"%s\"", comp_name, parent->comp_name, parent->children[0]->comp_name, annotation);
	...
	PIO_sub_comm = MPI_COMM_NULL;
	num_PIO_procs = -1;
	PIO_proc_ID_stride = -1;
	PIO_proc_mark = -1;
...
void Comp_comm_group_mgt_node::generate_PIO_comm_info() 
{
	int max_num_pio_proc = comp_comm_group_mgt_mgr->get_max_num_of_PIO_procs();

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, max_num_pio_proc > 0, "Software error in Comp_comm_group_mgt_node::generate_PIO_comm_info");
	if (PIO_sub_comm != MPI_COMM_NULL)
		return;

	num_PIO_procs = get_num_procs() > max_num_pio_proc? max_num_pio_proc : get_num_procs();
	PIO_proc_ID_stride = get_num_procs() / num_PIO_procs;
	PIO_proc_mark = (get_current_proc_local_id()%PIO_proc_ID_stride) == 0 && (get_current_proc_local_id()/PIO_proc_ID_stride) < num_PIO_procs? 1 : 0;
    MPI_Comm_split(comm_group, PIO_proc_mark, PIO_proc_mark, &PIO_sub_comm);
}
...
int Comp_comm_group_mgt_mgr::register_ensemble_set_of_comps(const char *comps_ensemble_set_name, int member_comp_id, int ensemble_size, const char *annotation)
{
    ...
    synchronize_comp_processes_for_API(member_comp_id, API_ID_ENSEMBLE_PROC_INST_INIT, member_comp_node->get_comm_group(), "synchorization for registering an ensemble set of component models", annotation);	
	...
	check_API_parameter_int(member_comp_id, API_ID_ENSEMBLE_PROC_INST_INIT, member_comp_node->get_comm_group(), "the number of component models", num_comps, "num_comps", annotation);
	for (int i = 0; i < num_comps; i ++) {
		check_API_parameter_string(member_comp_id, API_ID_ENSEMBLE_PROC_INST_INIT, member_comp_node->get_comm_group(), "names of component models", temp_comps_name[i], "comps_names", annotation);
	...
	check_API_parameter_int(member_comp_id, API_ID_ENSEMBLE_PROC_INST_INIT, member_comp_node->get_comm_group(), "the number of members in the ensemble", num_comps, "ensemble_size", annotation);
	...
	int new_set_comp_id = register_component(comps_ensemble_set_name, COMP_TYPE_ACTIVE_COUPLED, new_set_comm, global_node_array[0]->get_comp_id(), false, 0, 0, annotation);
...
int Comp_comm_group_mgt_mgr::register_component(const char *comp_name, const char *comp_type, MPI_Comm &comm, int parent_local_id, bool enabled_in_parent_coupling_gen, int change_dir, int ensemble_id, const char *annotation)
{
    int i;
    ...
    if (change_dir != 0) {
        ...
        if (change_dir == 2)
            //sprintf(new_dir, "%s/run/ensemble_%d/%s/%s/data", root_working_dir, ensemble_id, new_comp->get_comp_type(), new_comp->get_comp_name());
            sprintf(new_dir, "%s/run/%s/%s/%d/data", root_working_dir, new_comp->get_comp_type(), new_comp->get_comp_name(), ensemble_id);
        ...
        EXECUTION_REPORT(REPORT_ERROR, new_comp->get_comp_id(), dir != NULL, "Fail to change working directory for the component model \"%s\": the directory \"%s\" does not exist.", new_comp->get_comp_name(), new_dir);
            chdir(new_dir);
...
Comp_comm_group_mgt_node *Comp_comm_group_mgt_mgr::load_comp_info_from_XML(int host_comp_id, const char *comp_full_name, MPI_Comm comm)
{
	...
	Comp_comm_group_mgt_node *existing_comp_node = comp_comm_group_mgt_mgr->search_global_node(comp_full_name);
	...
	if (existing_comp_node != NULL) {
		delete pesudo_comp_node;
		return existing_comp_node;
	}
	
    pesudo_comp_node->reset_local_node_id((unique_comp_id_indx++)|TYPE_COMP_LOCAL_ID_PREFIX);
    global_node_array.push_back(pesudo_comp_node); 
