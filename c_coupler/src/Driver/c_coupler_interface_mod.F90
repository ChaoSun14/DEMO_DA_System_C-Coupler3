...
!*************************************************************
 ...
 MODULE CCPL_interface_mod
   ...
   public :: CCPL_get_sub_H2D_grid_id
   ...
   public :: CCPL_external_procedures_para_get_field_grid_ID
   public :: CCPL_external_procedures_para_get_field_decomp_ID
   ...
   public :: CCPL_external_procedures_para_get_ensemble_size
   public :: CCPL_external_procedures_para_get_ensemble_member_index
   public :: CCPL_register_datamodel_output_handler
   public :: CCPL_handle_normal_explicit_output
   public :: CCPL_register_field_instances_output_handler
   public :: CCPL_get_io_proc_num
   public :: CCPL_readin_field_from_dataFile
   public :: CCPL_register_datamodel_input_handler
   public :: CCPL_handle_normal_explicit_input
...
! 
   ...
   integer FUNCTION CCPL_get_sub_H2D_grid_id(super_grid_id, annotation)
     implicit none
     integer,          intent(in)            :: super_grid_id
     character(len=*), intent(in), optional  :: annotation
     integer                                 :: sub_grid_id

     if (present(annotation)) then
         call get_ccpl_sub_H2D_grid_id(super_grid_id, sub_grid_id, trim(annotation)//char(0))
     else
         call get_ccpl_sub_H2D_grid_id(super_grid_id, sub_grid_id, trim("")//char(0))
     endif
     CCPL_get_sub_H2D_grid_id = sub_grid_id

   END FUNCTION CCPL_get_sub_H2D_grid_id
 ...
 SUBROUTINE CCPL_get_comm_array_min_max_value_pos_double(comm, min_max_hint, values, pos1, pos2, pos3, pos4, annotation)
  ...
  integer FUNCTION CCPL_register_component(parent_id, comp_name, comp_type, comp_comm, considered_in_parent_coupling_gen, change_dir, annotation)
   ...
   integer                                 :: ensemble_nums, ensemble_id
   integer                                 :: parameter_nums
   character *1024                         :: ccpl_ens_index, ensemble_mem_id_str, ens_comp_name
   ...
   ensemble_nums = 0
   ensemble_id = 0 
   parameter_nums = iargc()
   if (parameter_nums .ge. 1) then
       call GETARG(parameter_nums, ccpl_ens_index) !ccpl_ens_index = CCPL_ensemble_*
       call get_ccpl_ensemble_comp_info(trim(comp_name)//char(0), ensemble_nums, ensemble_id, trim(ccpl_ens_index)//char(0), trim(local_annotation)//char(0))             
   endif
   ...
   if (parent_id .eq. -1) then
      ...
      ens_comp_name = comp_name   
      if (ensemble_nums .ne. 0 .and. ensemble_id .ne. 0) then
         write(ensemble_mem_id_str,*) ensemble_id
         ens_comp_name = trim(comp_name)//"_member"//trim(adjustl(ensemble_mem_id_str))
         if (present(change_dir) .and. change_dir) local_change_dir = 2
      endif
      call register_root_component(comp_comm, trim(ens_comp_name)//char(0), trim(comp_type)//char(0), trim(local_annotation)//char(0), comp_id, local_considered_in_parent_coupling_gen, local_change_dir, ensemble_id, trim(exe_name)//char(0))
   else
      if (ensemble_nums .ne. 0 .and. ensemble_id .ne. 0) then
         if (present(change_dir) .and. change_dir) local_change_dir = 2
      endif
      call register_component(parent_id, trim(comp_name)//char(0), trim(comp_type)//char(0), comp_comm, trim(local_annotation)//char(0), local_considered_in_parent_coupling_gen, local_change_dir, ensemble_id, comp_id)
   endif
   ...
   integer FUNCTION CCPL_register_MD_grid_via_multi_grids(comp_id, grid_name, sub_grid1_id, sub_grid2_id, sub_grid3_id, mask, decomp_id, annotation)
   implicit none
   integer, intent(in)                                               :: comp_id
   character(len=*), intent(in)                                      :: grid_name
   ...
   character(len=*), intent(in),                       optional      :: annotation
   integer,          intent(in), dimension(:), target, optional      :: mask
   integer, intent(in),                                optional      :: decomp_id
   integer,                      dimension(:), pointer               :: temp_mask, temp_int
   ...
   call initialize_CCPL_external_procedures_inst(instance_id, trim(inst_name)//char(0), trim(procedures_name)//char(0), trim(ptype)//char(0), &
          host_comp_id, trim(local_dl_name)//char(0), tmp_process_active, size_controls, size_field_inst, size_timers, tmp_control_vars, tmp_field_inst_ids, tmp_timers_id, trim(local_annotation)//char(0))
   ...
   integer FUNCTION CCPL_external_procedures_para_get_field_grid_ID(instance_id, field_name, annotation)
   implicit none
   integer,          intent(in)           :: instance_id
   character(len=*), intent(in)           :: field_name
   character(len=*), intent(in), optional :: annotation
   ...
   if (present(annotation)) then
      call get_CCPL_external_procedures_grid_id(grid_id, instance_id, trim(field_name)//char(0), trim(annotation)//char(0))
   else
      call get_CCPL_external_procedures_grid_id(grid_id, instance_id, trim(field_name)//char(0), trim("")//char(0))
   endif
   CCPL_external_procedures_para_get_field_grid_ID = grid_id

   END FUNCTION CCPL_external_procedures_para_get_field_grid_ID



   integer FUNCTION CCPL_external_procedures_para_get_field_decomp_ID(instance_id, field_name, annotation)
   implicit none
   integer,          intent(in)           :: instance_id
   character(len=*), intent(in)           :: field_name
   character(len=*), intent(in), optional :: annotation
   ...
   if (present(annotation)) then
      call get_CCPL_external_procedures_decomp_id(decomp_id, instance_id, trim(field_name)//char(0), trim(annotation)//char(0))
   else
      call get_CCPL_external_procedures_decomp_id(decomp_id, instance_id, trim(field_name)//char(0), trim("")//char(0))
   endif
   CCPL_external_procedures_para_get_field_decomp_id = decomp_id

   END FUNCTION CCPL_external_procedures_para_get_field_decomp_ID
...
if (do_abort) do_abort_int = 1
   ...
   integer FUNCTION CCPL_register_datamodel_output_handler(handler_name, num_field_instances, field_instance_ids, datamodel_name, implicit_or_explicit, output_grid_id, handler_output_h2d_grid_id, handler_output_v1d_grid_id, sampling_timer_id, annotation)
   implicit none
   ...
   integer,          intent(in), optional               :: output_grid_id
   integer,          intent(in), optional               :: handler_output_h2d_grid_id
   integer,          intent(in), optional               :: handler_output_v1d_grid_id
   integer,          intent(in), optional               :: sampling_timer_id
   character(len=*), intent(in), optional               :: annotation
   integer                                              :: handler_id, output_grid_id_temp, sampling_timer_id_temp
   integer                                              :: handler_output_h2d_grid_id_temp, handler_output_v1d_grid_id_temp
   character *1024                                      :: handler_annotation

   if (present(output_grid_id)) then
      output_grid_id_temp = output_grid_id
   else
      output_grid_id_temp = -1
   endif

   if (present(handler_output_h2d_grid_id)) then 
      handler_output_h2d_grid_id_temp = handler_output_h2d_grid_id
   else
      handler_output_h2d_grid_id_temp = -1
   endif

   if (present(handler_output_v1d_grid_id)) then
      handler_output_v1d_grid_id_temp = handler_output_v1d_grid_id
   else
      handler_output_v1d_grid_id_temp = -1
   endif

   if (present(sampling_timer_id)) then
      sampling_timer_id_temp = sampling_timer_id
   else
      sampling_timer_id_temp = -1
   endif
   ...
   call register_datamodel_output_handler(handler_name, handler_id, num_field_instances, field_instance_ids, trim(datamodel_name)//char(0), implicit_or_explicit, output_grid_id_temp, handler_output_h2d_grid_id_temp, handler_output_v1d_grid_id_temp, sampling_timer_id_temp, size(field_instance_ids), handler_annotation)
   ...
   integer FUNCTION CCPL_register_field_instances_output_handler(num_field_instances, field_instance_ids, file_name, file_type, implicit_or_explicit, sampling_timer_id, output_timer_id, file_timer_id, output_grid_id, inst_or_aver, float_datatype, integer_datatype, annotation)
   implicit none
   integer,          intent(in)                         :: num_field_instances
   integer,          intent(in), dimension(:)           :: field_instance_ids
   character(len=*), intent(in)                         :: file_name
   ...
   logical,          intent(in)                         :: implicit_or_explicit
   integer,          intent(in), optional               :: sampling_timer_id
   integer,          intent(in), optional               :: output_timer_id
   integer,          intent(in), optional               :: file_timer_id
   integer,          intent(in), optional               :: output_grid_id
   integer,          intent(in), optional               :: inst_or_aver
   character(len=*), intent(in), optional               :: float_datatype
   character(len=*), intent(in), optional               :: integer_datatype
   character(len=*), intent(in), optional               :: annotation

   integer                                              :: inst_or_aver_temp
   integer                                              :: handler_id, output_grid_id_temp, file_timer_id_temp, sampling_timer_id_temp, i
   integer                                              :: output_timer_id_temp
   integer                                              :: handler_output_v1d_grid_id_temp, handler_output_h2d_grid_id_temp
   character *1024                                      :: float_datatype_temp, integer_datatype_temp
   character *1024                                      :: handler_annotation
   ...
   if (present(output_timer_id)) then
    output_timer_id_temp = output_timer_id
   else
    output_timer_id_temp = -1
   endif

   if (present(file_timer_id)) then
    file_timer_id_temp = file_timer_id
   else
    file_timer_id_temp = -1
   endif
   ...
   if (present(float_datatype)) then
    float_datatype_temp = trim(float_datatype)//char(0)
   else
    float_datatype_temp = ""//char(0)
   endif
   if (present(integer_datatype)) then
    integer_datatype_temp = trim(integer_datatype)//char(0)
   else
    integer_datatype_temp = ""//char(0)
   endif
   ...
   if (present(inst_or_aver)) then
     inst_or_aver_temp = inst_or_aver
   else
     inst_or_aver_temp = 0
   end if
   
   call register_field_instances_output_handler(handler_id, num_field_instances, field_instance_ids, trim(file_name)//char(0), trim(file_type)//char(0), implicit_or_explicit, sampling_timer_id_temp, output_timer_id_temp, file_timer_id_temp, output_grid_id_temp, inst_or_aver_temp, float_datatype_temp, integer_datatype_temp, size(field_instance_ids), handler_annotation)

   CCPL_register_field_instances_output_handler = handler_id
   
   END FUNCTION CCPL_register_field_instances_output_handler



   integer FUNCTION CCPL_get_io_proc_num(comp_id)
   implicit none
   integer,          intent(in)                         :: comp_id
   integer                                              :: io_proc_num

   call get_io_proc_num(comp_id, io_proc_num)
   CCPL_get_io_proc_num = io_proc_num

   END FUNCTION CCPL_get_io_proc_num
   ...
   logical FUNCTION CCPL_readin_field_from_dataFile(field_instance_id, data_file_name, file_type, field_name_in_file, necessity, grid_id_for_file, value_min_bound, value_max_bound, annotation)
    ...
    character(len=*), intent(in)                         :: data_file_name
    character(len=*), intent(in)                         :: file_type
    character(len=*), intent(in), optional               :: field_name_in_file
    logical,          intent(in), optional               :: necessity
    integer,          intent(in), optional               :: grid_id_for_file
    real(r8),         intent(in), optional               :: value_min_bound
    real(r8),         intent(in), optional               :: value_max_bound
    character(len=*), intent(in), optional               :: annotation
    character *1024                                      :: local_annotation, field_name_in_file_temp
    integer         :: grid_id_for_file_temp, necessity_temp
    real            :: value_min_bound_temp, value_max_bound_temp
    integer :: have_max_bound, have_min_bound
    integer :: successful

    local_annotation = ""
    if (present(annotation)) local_annotation = annotation
    if (present(grid_id_for_file)) then
       grid_id_for_file_temp = grid_id_for_file
    else
       grid_id_for_file_temp = -1
    endif
    
    if (present(value_min_bound)) then
    have_min_bound = 1
    value_min_bound_temp = value_min_bound
    else 
    have_min_bound = 0
    endif

    if (present(value_max_bound)) then
    have_max_bound = 1
    value_max_bound_temp = value_max_bound
    else
    have_max_bound = 0
    endif

    if (present(necessity) .and. necessity) then
    necessity_temp = 1
    else
    necessity_temp = 0
    endif

    field_name_in_file_temp = trim("")//char(0)
    if (present(field_name_in_file)) then
    field_name_in_file_temp = trim(field_name_in_file)//char(0)
    endif

    call read_in_field_from_datafile(field_instance_id, trim(data_file_name)//char(0), trim(file_type)//char(0), field_name_in_file_temp, necessity_temp, grid_id_for_file_temp, value_min_bound_temp, value_max_bound_temp, have_min_bound, have_max_bound, trim("real8")//char(0), successful, trim(local_annotation)//char(0))

    CCPL_readin_field_from_dataFile = .false.
    if (successful .eq. 1) CCPL_readin_field_from_dataFile = .true.

    END FUNCTION CCPL_readin_field_from_dataFile



    integer FUNCTION CCPL_register_datamodel_input_handler(handler_name, field_instances_ids, num_fields, config_input_instance_name, input_timer_id, necessity, field_connected_status, annotation)
    implicit none
    character(len=*), intent(in)                         :: handler_name
    integer,          intent(in), dimension(:)           :: field_instances_ids
    integer,          intent(in)                         :: num_fields
    character(len=*), intent(in)                         :: config_input_instance_name
    integer,          intent(in)                         :: input_timer_id
    logical, intent(in), dimension(:), target, optional  :: necessity
    logical, intent(in), dimension(:), target, optional  :: field_connected_status
    character(len=*), intent(in), optional               :: annotation
    character *1024                                      :: local_annotation
    logical, dimension(:), pointer:: necessity_temp, field_connected_status_temp, temp_int, temp_int2
    integer :: handler_id, necessity_size, field_connected_status_size

    allocate(temp_int(1))
    field_connected_status_temp=>temp_int
    necessity_temp=>temp_int2
    necessity_size = 0
    field_connected_status_size = 0

    if (present(necessity)) then
      necessity_temp=>necessity
      necessity_size = size(necessity)
    endif

    if (present(field_connected_status)) then
      field_connected_status_temp=>field_connected_status
      field_connected_status_size = 0
    endif

    if (present(annotation)) then
      local_annotation = trim(annotation)//char(0)
    else
      local_annotation = trim("")//char(0)
    endif

    call register_datamodel_input_handler(handler_id, handler_name, field_instances_ids, num_fields, size(field_instances_ids), input_timer_id, necessity_size, field_connected_status_size, config_input_instance_name, necessity_temp, field_connected_status_temp, local_annotation)
    CCPL_register_datamodel_input_handler = handler_id

    END FUNCTION CCPL_register_datamodel_input_handler



    logical FUNCTION CCPL_handle_normal_explicit_input(handler_id, annotation)
    ...
    call handle_normal_explicit_input(handler_id, handler_annotation)
    CCPL_handle_normal_explicit_input = .true.

    END FUNCTION CCPL_handle_normal_explicit_input



   INTEGER FUNCTION CCPL_ensemble_procedures_inst_init(member_comp_id, inst_name, &
   ...
   character *2048                             :: local_annotation
   integer, dimension(:), pointer              :: tmp_control_vars
   integer, dimension(:), pointer              :: tmp_int_array 
   integer                                     :: size_controls
   integer                                     :: size_field_inst, instance_id
   integer                                     :: ensemble_nums, ensemble_id
   integer                                     :: parameter_nums
   character *1024                             :: ccpl_ens_index
    ...
    if (parameter_nums .ge. 1) then
        call GETARG(parameter_nums, ccpl_ens_index) !ccpl_ens_index = CCPL_ensemble_*
        call get_ccpl_ensemble_comp_info(trim(inst_name)//char(0), ensemble_nums, ensemble_id, ccpl_ens_index)             
    end if
   ...
   size_field_inst = size(field_inst_ids)
   call initialize_CCPL_ensemble_procedures_inst_init(instance_id, trim(inst_name)//char(0), member_comp_id, ensemble_nums, ensemble_id, &
          size_field_inst, size_controls, field_inst_ids, tmp_control_vars, trim(local_annotation)//char(0))
   ...
   SUBROUTINE CCPL_ensemble_procedures_inst_run(instance_id, annotation)
     implicit none
     integer,          intent(in)                :: instance_id
     character(len=*), intent(in), optional      :: annotation
     character *2048                             :: local_annotation
     ...
     local_annotation = ""
     if (present(annotation)) local_annotation = annotation
     call run_ccpl_ensemble_procedures_inst(instance_id, trim(local_annotation)//char(0))

   END SUBROUTINE CCPL_ensemble_procedures_inst_run



   integer FUNCTION CCPL_external_procedures_para_get_ensemble_size(external_inst_id, annotation)
   implicit none
   integer,          intent(in)           :: external_inst_id
   character(len=*), intent(in), optional :: annotation
   integer                                :: ensemble_num

   if (present(annotation)) then
      call get_CCPL_ensemble_procedures_external_inst_ensemble_size(ensemble_num, external_inst_id, trim(annotation)//char(0))
   else
      call get_CCPL_ensemble_procedures_external_inst_ensemble_size(ensemble_num, external_inst_id, trim("")//char(0))
   endif
   CCPL_external_procedures_para_get_ensemble_size = ensemble_num

   END FUNCTION CCPL_external_procedures_para_get_ensemble_size


   integer FUNCTION CCPL_external_procedures_para_get_ensemble_member_index(external_inst_id, annotation)
   implicit none
   integer,          intent(in)           :: external_inst_id
   character(len=*), intent(in), optional :: annotation
   integer                                :: ensemble_id
   ...
   if (present(annotation)) then
      call get_ccpl_ensemble_procedures_external_inst_member_index(ensemble_id, external_inst_id, trim(annotation)//char(0))
   else
      call get_ccpl_ensemble_procedures_external_inst_member_index(ensemble_id, external_inst_id, trim("")//char(0))
   endif
   CCPL_external_procedures_para_get_ensemble_member_index = ensemble_id

   END FUNCTION CCPL_external_procedures_para_get_ensemble_member_index
