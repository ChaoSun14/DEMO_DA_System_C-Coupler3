...
void get_API_hint(int comp_id, int API_id, char *API_label)
{
    switch(API_id) {
		...
		case API_ID_FIELD_MGT_GET_FIELD_INST_GRID_ID:
            ...
            break;			
		case API_ID_GRID_MGT_GET_SUB_H2D_GRID_ID:
            sprintf(API_label, "CCPL_get_sub_H2D_grid_id");
            break;
		...
		case API_ID_EXTERNAL_PROC_INST_GET_GRID_ID:
			sprintf(API_label, "CCPL_external_procedures_para_get_field_grid_ID");
			break;
		case API_ID_EXTERNAL_PROC_INST_GET_DECOMP_ID:
			sprintf(API_label, "CCPL_external_procedures_para_get_field_decomp_ID");
			break;
        ...
        case API_ID_ENSEMBLE_PROC_INST_RUN:
            ...
            break;
        case API_ID_ENSEMBLE_PROC_INST_GET_ENSEMBLE_NUM:
            sprintf(API_label, "CCPL_external_procedures_para_get_ensemble_size");
            break;
        case API_ID_ENSEMBLE_PROC_INST_GET_ENSEMBLE_ID:
            sprintf(API_label, "CCPL_external_procedures_para_get_ensemble_member_index");
            break;
        ...
        case API_ID_HANDLER_DATAMODEL_OUTPUT:
            ...
            break;
        case API_ID_HANDLER_DATAMODEL_INPUT:
            sprintf(API_label, "CCPL_register_datamodel_input_handler");
            break;
        case API_ID_HANDLE_NORMAL_EXPLICIT_OUTPUT:
            sprintf(API_label, "CCPL_handle_normal_explicit_output");
            break;
        case API_ID_HANDLER_FIELD_INSTANCES_OUTPUT:
            sprintf(API_label, "CCPL_register_field_instances_output_handler");
	   case API_ID_NORMAL_TIMING_START:
	    ...
	    case API_ID_NORMAL_TIMING_STOP:
            ...
            break;
        case API_ID_HANDLER_DATA_FILE_INPUT:
            sprintf(API_label, "CCPL_readin_field_from_dataFile");
            break;
        case API_ID_HANDLE_NORMAL_EXPLICIT_INPUT:
            sprintf(API_label, "CCPL_handle_normal_explicit_input");
            break;
        default:
...
void synchronize_comp_processes_for_API(int comp_id, int API_id, MPI_Comm comm, const char *hint, const char *annotation)
{
    ...
    if (hint != NULL) {
        EXECUTION_REPORT_LOG(REPORT_LOG, comp_id, true, "Before the MPI_barrier for synchronizing all processes of a communicator for %s at C-Coupler API \"%s\" with model code annotation \"%s\", comm:%lx", hint, API_label_local, annotation, comm);    
    }
    else EXECUTION_REPORT_LOG(REPORT_LOG, comp_id, true, "Before the MPI_barrier for synchronizing all processes of a communicator at C-Coupler API \"%s\" with model code annotation \"%s\", comm:%lx", API_label_local, annotation, comm);
    MPI_Barrier(comm);
    if (hint != NULL) {
...
char *check_and_aggregate_local_grid_data(int comp_id, int API_id, MPI_Comm comm, const char *hint, int grid_size, int array_size, int data_type_size, char *array_value, 
                                          const char *parameter_name, int num_local_cells, const int *local_cells_global_index, int &grid_data_size, const char *annotation)
{
    char API_label[NAME_STR_SIZE];
    ...
    EXECUTION_REPORT(REPORT_ERROR,-1, MPI_Gatherv((int*)local_cells_global_index, num_local_cells, MPI_INT, all_local_cells_global_index, counts_for_cell_index, displs_for_cell_index, MPI_INT, 0, comm) == MPI_SUCCESS);
    if (data_type_size == 4)
		...
		EXECUTION_REPORT(REPORT_ERROR,-1, MPI_Gatherv(array_value, array_size, MPI_DOUBLE, all_array_values, counts_for_array, displs_for_array, MPI_DOUBLE, 0, comm) == MPI_SUCCESS);
	else EXECUTION_REPORT(REPORT_ERROR,-1, MPI_Gatherv(array_value, array_size, MPI_CHAR, all_array_values, counts_for_array, displs_for_array, MPI_CHAR, 0, comm) == MPI_SUCCESS);
        ...
        if (grid_data_size > 0) {
            ...
            for (int i = 0; i < num_processes; i ++)
                for (int j = 0; j < counts_for_cell_index[i]; j ++) {
                    int global_index = all_local_cells_global_index[displs_for_cell_index[i]+j];
                    if (grid_data_mark[global_index] == 0) {
                        ...
                        EXECUTION_REPORT(REPORT_ERROR, comp_id, is_the_same, "Error happens when calling the API \"%s\" for %s: the grid data (\"%s\") of some common cells (%d) is not the same among the processes. Please check the model code related to the annotation \"%s\"", API_label, hint, parameter_name, global_index, annotation);
                    }
                }
            delete [] grid_data_mark;
            if (report_error_enabled) {
                EXECUTION_REPORT(REPORT_ERROR, comp_id, all_local_cells_global_index[0] == 0, "Error happens when calling the API \"%s\" for %s: no process provide grid data (\"%s\") for the first grid cell. Please check the model code related to the annotation \"%s\"", API_label, hint, parameter_name, annotation);
                EXECUTION_REPORT(REPORT_ERROR, comp_id, all_local_cells_global_index[num_total_cells-1] == grid_size - 1, "Error happens when calling the API \"%s\" for %s: no process provide grid data (\"%s\") for the last (No. %d) grid cell. Please check the model code related to the annotation \"%s\"", API_label, hint, parameter_name, grid_size, annotation);
                for (int i = 1; i < num_total_cells; i ++)                    
                    EXECUTION_REPORT(REPORT_ERROR, comp_id, all_local_cells_global_index[i] == all_local_cells_global_index[i-1]+1, "Error happens when calling the API \"%s\" for %s: no process provide grid data (\"%s\") for the No. %d grid cell. Please check the model code related to the annotation \"%s\"", API_label, hint, parameter_name, all_local_cells_global_index[i-1]+1, annotation);
            }
        }
    }
...
void check_API_parameter_data_array(int comp_id, int API_id, MPI_Comm comm, const char *hint, int array_size, int data_type_size, const char *array_value, const char *parameter_name, const char *annotation)
{
    ...
    total_checksum = calculate_checksum_of_array(array_value, array_size, data_type_size, NULL, MPI_COMM_NULL);
...
void get_required_XML_element(int comp_id, const char *XML_file_name, const char *XML_file_annotation, TiXmlNode **current_XML_node, const char *node_name, bool be_active, bool be_unique, bool necessary)
{
	while (*current_XML_node != NULL) {
		if ((*current_XML_node)->Type() != TiXmlNode::TINYXML_ELEMENT)
			continue;
		TiXmlElement *current_XML_element = (*current_XML_node)->ToElement();
		char status_annotation[NAME_STR_SIZE];
		sprintf(status_annotation, "the status of \"%s\"", node_name);
		if (be_active && !is_XML_setting_on(comp_id, current_XML_element, XML_file_name, status_annotation, XML_file_annotation)) {
			(*current_XML_node) = (*current_XML_node)->NextSibling();
			continue;
		}		
		if (node_name != NULL && !words_are_the_same(node_name, current_XML_element->Value())) {
			(*current_XML_node) = (*current_XML_node)->NextSibling();
			continue;
		}
		if (!be_unique)
			break;
		TiXmlNode *temp_XML_node = (*current_XML_node)->NextSibling();
		get_required_XML_element(comp_id, XML_file_name, XML_file_annotation, &temp_XML_node, node_name, be_active, be_unique, false);
		if (node_name != NULL)
			EXECUTION_REPORT(REPORT_ERROR, comp_id, temp_XML_node == NULL, "ERROR happens when getting the unique node named \"%s\" from the XML file \"%s\" that is for %s: there are multiple XML nodes with this name. Please verify the XML file.", node_name, XML_file_name, XML_file_annotation);
		else EXECUTION_REPORT(REPORT_ERROR, comp_id, temp_XML_node == NULL, "ERROR happens when getting the unique node from the XML file \"%s\" that is for %s: there are multiple XML nodes with this name. Please verify the XML file.", XML_file_name, XML_file_annotation);
		break;
	}

	if (necessary)
		EXECUTION_REPORT(REPORT_ERROR, -1, *current_XML_node != NULL, "ERROR happens when parsing the XML file \"%s\" for \"%s\": it does not have an active XML node named \"%s\". Please verify the XML file. ", XML_file_name, XML_file_annotation, node_name);
}
                                             ...
                                             int root_proc_global_id_dst_comp, MPI_Comm comm_dst_comp, char **array, long &array_size)
{
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "transfer_array_from_one_comp_to_another");
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "transfer_array_from_one_comp_to_another");
}
                              ...
                              int data_type_size, int *all_array_size, void **global_array, long &global_size, MPI_Comm comm)
{
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_start(TIMING_TYPE_COMPUTATION, -1, -1, "gather_array_in_one_comp");
    ...
    comp_comm_group_mgt_mgr->get_root_component_model()->get_performance_timing_mgr()->performance_timing_stop(TIMING_TYPE_COMPUTATION, -1, -1, "gather_array_in_one_comp");
}
