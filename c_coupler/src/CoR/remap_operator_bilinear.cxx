...
void Remap_operator_bilinear::initialize()
{
...
int Remap_operator_bilinear::search_at_least_16_nearnest_src_points_for_bilinear(double *dst_cell_center_values,
                                                                                 long src_cell_index, long dst_cell_index,
                                                                                 ...
                                                                                 double &near_optimal_threshold_distance)
{
    ...
    while (num_points_within_threshold_distance < 16) {
		...
		dynamic_search_nearest_points_var_distance(dst_cell_index, num_points_within_threshold_distance, found_nearest_points_src_indexes, found_nearest_points_distance, current_threshold_distance, -1, true);
                                                            ...
                                                            long *index_of_selected_src_point_in_each_quadrant)
{
	...
	long global_index_buffer[256*256];
    ...
    for (i = 0; i < 4; i ++) {
		sort_grid_nearest_points(distances_of_src_points_in_each_quadrant[i], indexes_of_src_points_in_each_quadrant[i], global_index_buffer, num_src_points_in_each_quadrant[i], src_grid);
...
void Remap_operator_bilinear::compute_remap_weights_of_one_dst_cell(long dst_cell_index)
{
    ...
    double vertex_coord_values_dst[65536], vertex_coord_values_src[65536];
	...
	if (src_grid->get_local_cell_global_indexes() != NULL && dst_grid->get_local_cell_global_indexes() != NULL && dst_grid->get_local_cell_global_indexes()[dst_cell_index] == 0)
		EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "whywhy1: %d (%lf %lf): %d (%lf %lf)", src_grid->get_local_cell_global_indexes()[src_cell_index], src_cell_center_values[0], src_cell_center_values[1], dst_grid->get_local_cell_global_indexes()[dst_cell_index], dst_cell_center_values[0], dst_cell_center_values[1]); 
