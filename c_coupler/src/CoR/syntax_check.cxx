...
void check_is_parameter_string_type(const char *function, int para_indx, Remap_statement_operand *operand, const char *annotation)
{
    ...
    sprintf(tmp_str, "No. %d", para_indx);
...
long get_size_value_from_parameter(const char *function, int para_indx, Remap_statement_operand *operand, const char *annotation)
{
    ...
    if (annotation == NULL)
        sprintf(text, "the No. %d input parameter of function %s must be an integer\n", para_indx, function);
    else sprintf(text, "the No. %d input parameter of function %s must be an positive integer or a dimension length in IO file, which is %s\n", para_indx, function, annotation);
...
long get_int_value_from_parameter(const char *function, int para_indx, Remap_statement_operand *operand, const char *annotation)
{
    ...
    if (annotation == NULL)
        sprintf(text, "the No. %d input parameter of function %s must be an integer\n", para_indx, function);
    else sprintf(text, "the No. %d input parameter of function %s must be an integer, which is %s\n", para_indx, function, annotation);
...
double get_float_value_from_parameter(const char *function, int para_indx, Remap_statement_operand *operand, const char *annotation)
{
    ...
    if (annotation == NULL)
        sprintf(text, "the No. %d input parameter of function %s must be a float value\n", para_indx, function);
    else sprintf(text, "the No. %d input parameter of function %s must be a float value, which is %s\n", para_indx, function, annotation);
...
void check_parameter_object_type(const char *function, int para_indx, Remap_statement_operand *operand, const char *annotation, const char *object_type)
{
    ...
    if (para_indx > 0)
        if (annotation == NULL)
            sprintf(text, "the No. %d input parameter of function %s must be a %s\n", para_indx, function, object_type);
        else sprintf(text, "the No. %d input parameter of function %s must be a %s, which is %s\n", para_indx, function, object_type, annotation);
    else
        if (annotation == NULL)
