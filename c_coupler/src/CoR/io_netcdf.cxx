...
IO_netcdf::IO_netcdf(const char *object_name, const char *file_name, const char *format, bool io_with_time_info)
{
    ...
    if (words_are_the_same(format, "r"))
        ...
        rcode = nc_create(file_name, NC_CLOBBER|NC_64BIT_OFFSET, &ncfile_id);
        report_nc_error();
        nc_enddef(ncfile_id);        
    }
...
{
    int num_sized_sub_grids, num_leaf_grids, num_masked_sub_grids, i, dim_ncid;
    ...
    for (i = 0; i < num_sized_sub_grids; i ++)
        if (sized_grids_map.find(sized_sub_grids[i]) == sized_grids_map.end()) {
            if (write_grid_name) {
                ...
                else if (sized_sub_grids[i]->get_is_H2D_grid())
            ...
            EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "define dim %s  %d for grid \"%s\" in ncfile %s", tmp_string, associated_grid->get_grid_size(), sized_sub_grids[i]->get_grid_name(), file_name);
            report_nc_error();
    ...
    for (i = 0; i < num_leaf_grids; i ++) {
        ...
        if (grid_data_field != NULL && (!grid_data_field->get_coord_value_grid()->get_are_vertex_values_set_in_default() || grid_data_field->get_grid_data_field()->required_data_size > 0)) {
...
void IO_netcdf::write_remap_weights(Remap_operator_basis *remap_operator, Remap_weight_sparse_matrix *weight_sparse_matrix)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, remap_operator != NULL && remap_operator->get_src_grid()->get_is_H2D_grid(), "remap software error1 in write_remap_weights of netcdf file\n");
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, remap_grid_manager->search_remap_grid_with_grid_name(remap_operator->get_src_grid()->get_grid_name()) != NULL && remap_grid_manager->search_remap_grid_with_grid_name(remap_operator->get_dst_grid()->get_grid_name()) != NULL, "Software error in IO_netcdf::write_remap_weights");

    remap_grid_src = remap_grid_manager->search_remap_grid_with_grid_name(remap_operator->get_src_grid()->get_grid_name());
    remap_grid_dst = remap_grid_manager->search_remap_grid_with_grid_name(remap_operator->get_dst_grid()->get_grid_name());
    ...
    if (execution_phase_number == 1) {
        ...
        area_or_volumn_a = remap_operator->get_src_grid()->get_area_or_volumn();
        area_or_volumn_b = remap_operator->get_dst_grid()->get_area_or_volumn();
        rcode = nc_open(file_name, NC_WRITE, &ncfile_id);
        rcode = nc_redef(ncfile_id);
        report_nc_error();
        rcode = nc_def_dim(ncfile_id, "n_a", remap_operator->get_src_grid()->get_grid_size(), &dim_ncid_n_a);
        report_nc_error();
        rcode = nc_def_dim(ncfile_id, "n_b", remap_operator->get_dst_grid()->get_grid_size(), &dim_ncid_n_b);
        report_nc_error();
		...
		if (remap_grid_src->are_all_vertex_fields_specified_by_user()) {
	        rcode = nc_def_dim(ncfile_id, "nv_a", remap_operator->get_src_grid()->get_num_vertexes(), &dim_ncid_nv_a);
	        report_nc_error();            
		...
		if (remap_grid_dst->are_all_vertex_fields_specified_by_user()) {
	        rcode = nc_def_dim(ncfile_id, "nv_b", remap_operator->get_dst_grid()->get_num_vertexes(), &dim_ncid_nv_b);
	        report_nc_error();
        ...
        EXECUTION_REPORT(REPORT_ERROR, -1, remap_operator->get_src_grid()->get_grid_center_field(COORD_LABEL_LON) != NULL &&
                         remap_operator->get_src_grid()->get_grid_center_field(COORD_LABEL_LAT) != NULL, 
                         "remap software error3 in write_remap_weights of netcdf file\n");
        EXECUTION_REPORT(REPORT_ERROR, -1, remap_operator->get_dst_grid()->get_grid_center_field(COORD_LABEL_LON) != NULL &&
                         remap_operator->get_dst_grid()->get_grid_center_field(COORD_LABEL_LAT) != NULL, 
                         "remap software error3 in write_remap_weights of netcdf file\n");
        EXECUTION_REPORT(REPORT_ERROR, -1, remap_operator->get_src_grid()->get_grid_mask_field() != NULL &&
                         remap_operator->get_dst_grid()->get_grid_mask_field() != NULL, 
                         "remap software error7 in write_remap_weights of netcdf file\n");
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, remap_operator->get_src_grid()->get_grid_center_field(COORD_LABEL_LAT)->get_grid_data_field()->required_data_size == remap_operator->get_src_grid()->get_grid_size(), "Software error in IO_netcdf::write_remap_weights");
        rcode = nc_put_var_double(ncfile_id, yc_a_id, (double*)remap_operator->get_src_grid()->get_grid_center_field(COORD_LABEL_LAT)->get_grid_data_field()->data_buf);
        report_nc_error();
        rcode = nc_put_var_double(ncfile_id, xc_a_id, (double*)remap_operator->get_src_grid()->get_grid_center_field(COORD_LABEL_LON)->get_grid_data_field()->data_buf);
        report_nc_error();
        rcode = nc_put_var_double(ncfile_id, yc_b_id, (double*)remap_operator->get_dst_grid()->get_grid_center_field(COORD_LABEL_LAT)->get_grid_data_field()->data_buf);
        report_nc_error();
        rcode = nc_put_var_double(ncfile_id, xc_b_id, (double*)remap_operator->get_dst_grid()->get_grid_center_field(COORD_LABEL_LON)->get_grid_data_field()->data_buf);
        report_nc_error();
		if (remap_grid_src->are_all_vertex_fields_specified_by_user()) {
	        rcode = nc_put_var_double(ncfile_id, yv_a_id, (double*)remap_operator->get_src_grid()->get_grid_vertex_field(COORD_LABEL_LAT)->get_grid_data_field()->data_buf);
	        report_nc_error();
	        rcode = nc_put_var_double(ncfile_id, xv_a_id, (double*)remap_operator->get_src_grid()->get_grid_vertex_field(COORD_LABEL_LON)->get_grid_data_field()->data_buf);
	        report_nc_error();
		}
		if (remap_grid_dst->are_all_vertex_fields_specified_by_user()) {
	        rcode = nc_put_var_double(ncfile_id, yv_b_id, (double*)remap_operator->get_dst_grid()->get_grid_vertex_field(COORD_LABEL_LAT)->get_grid_data_field()->data_buf);
    	    report_nc_error();
        	rcode = nc_put_var_double(ncfile_id, xv_b_id, (double*)remap_operator->get_dst_grid()->get_grid_vertex_field(COORD_LABEL_LON)->get_grid_data_field()->data_buf);
	        report_nc_error();
    	}
        temp_int_values = new int [remap_operator->get_src_grid()->get_grid_size()];
        for (j = 0; j < remap_operator->get_src_grid()->get_grid_size(); j ++)
            if (((bool*)(remap_operator->get_src_grid()->get_grid_mask_field()->get_grid_data_field()->data_buf))[j])
                temp_int_values[j] = 1;
        ...
        for (j = 0; j < remap_operator->get_dst_grid()->get_grid_size(); j ++)
            if (((bool*)(remap_operator->get_dst_grid()->get_grid_mask_field()->get_grid_data_field()->data_buf))[j])
                temp_int_values[j] = 1;
            else temp_int_values[j] = 0;		
...
void IO_netcdf::put_global_attr(const char *text_title, const void *attr_value, const char *local_data_type, const char *nc_data_type, int size)
{
    ...
    if (words_are_the_same(nc_data_type, DATA_TYPE_STRING))
        ...
        nc_datatype = NC_INT;
    else if (words_are_the_same(nc_data_type, DATA_TYPE_LONG))
        nc_datatype = NC_INT;
    ...
    else if (words_are_the_same(local_data_type, DATA_TYPE_INT))
        rcode = nc_put_att_int(ncfile_id, NC_GLOBAL, text_title, NC_INT, size, (const int*)attr_value);
    else if (words_are_the_same(local_data_type, DATA_TYPE_LONG))
        rcode = nc_put_att_int(ncfile_id, NC_GLOBAL, text_title, NC_INT, size*2, (const int*)attr_value);
...
bool IO_netcdf::get_file_field_attribute(const char *field_name, const char *attribute_name, char *attribute_value, char *data_type)
{
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, strlen(data_type) < 16, "Software error IO_netcdf::get_file_field_attribute");
...
bool IO_netcdf::get_file_field_string_attribute(const char *field_name, const char *attribute_name, char *attribute_value, char *data_type, MPI_Comm comm, bool is_root_proc)
{
    ...
    if (comm != MPI_COMM_NULL) {
        ...
        MPI_Bcast(data_type, 16, MPI_CHAR, 0, comm);
    }
    ...
    *field_size = -1;
    ...
    if (is_root_proc) {
        ...
        if (rcode != NC_NOERR) {
			...
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, strlen(data_type) < 16, "Software error IO_netcdf::read_file_field");
...
void IO_netcdf::read_remap_weights(Remap_weight_of_strategy_class *remap_weights, Remap_strategy_class *remap_strategy, bool read_weight_values)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, remap_weights->get_data_grid_src()->get_is_H2D_grid() &&
...
bool IO_netcdf::get_field_datatype(const char *field_name, char *var_data_type)
{
    int variable_id = NC_GLOBAL;
    nc_type nc_data_type;
    ...
    rcode = nc_inq_vartype(ncfile_id, variable_id, &nc_data_type);
    if (rcode != NC_NOERR) {
        rcode = nc_close(ncfile_id);
        report_nc_error();
        return false;
