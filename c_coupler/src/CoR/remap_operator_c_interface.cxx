                           ...
                           int num_polygon_vertexes)
{
    ...
    for (i = 0, j = 0; i < num_vertexes_group1; i ++) {
        ...
        if (polygon_vertexes_src_cell_indexes)
...
//    printf("angle is %lf: %lf\n", angle, tmp);
    ...
    if (check_angle)
        EXECUTION_REPORT(REPORT_ERROR, -1, angle >= 0, "remap software error in compute_angle_of_great_arcs, (%lf, %lf)(%lf, %lf)(%lf, %lf) angle is %lf: %0.18lf\n", lon_point1, lat_point1, lon_point2, lat_point2, lon_point3, lat_point3, angle, tmp);
    return angle;
}
                                         ...
                                         double *vertexes_lats)
{
    ...
    if (area < 0 && (fabs(area) < eps || num_vertexes == 3))
        area = -area;
    EXECUTION_REPORT(REPORT_ERROR, -1, area >= 0, "remap software error in compute_area_of_sphere_cell, area = %lf\n", area);
    return area;
}
