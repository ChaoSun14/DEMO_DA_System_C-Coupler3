...
void seperate_cells_in_children_tiles(int num_cells, H2D_grid_cell_search_cell **cells, double center_lon, double center_lat, 
                                      double dlon, double dlat, int *num_cells_in_children, long *local_index_buffer, bool is_sphere_grid)
{
    ...
    for (i = 0; i < num_cells; i ++) {
        ...
        if (diff_lon < 0) {
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, is_sphere_grid, "Software error in seperate_cells_in_children_tiles");
            diff_lon += 360;
		}
        ...
        EXECUTION_REPORT(REPORT_ERROR, -1, indx_at_lon >= 0 && indx_at_lon < TILE_DIVIDE_FACTOR && indx_at_lat >= 0 && indx_at_lat < TILE_DIVIDE_FACTOR, "Software error1 in H2D_grid_cell_search_tile::seperate_cells_in_children_tiles %d %d %d", indx_at_lon, indx_at_lon, TILE_DIVIDE_FACTOR);
        child_indx = indx_at_lat*TILE_DIVIDE_FACTOR+indx_at_lon;
        num_cells_in_children[child_indx] ++;
        local_index_buffer[i] = child_indx;
    }
}
...
H2D_grid_cell_search_cell::H2D_grid_cell_search_cell(int cell_index, double center_lon, double center_lat, bool mask, 
                                                       int num_vertex, const double *vertex_lons, const double *vertex_lats, int edge_type, bool is_sphere_grid)
{
    int i, j;
	...
	this->is_sphere_grid = is_sphere_grid;
    ...
    if (num_vertex > 0) {
        ...
        if (this->num_vertex > 0) {
            ...
            for (i = 0; i < this->num_vertex; i ++) {
                ...
                if (max_lon - min_lon > 180 && min_lon < 180 && this->vertex_lons[i] < 180 && is_sphere_grid)
            ...
            if (bounding_circle_center_lon >= 360 && is_sphere_grid)
...
H2D_grid_cell_search_tile::H2D_grid_cell_search_tile(int num_cells, H2D_grid_cell_search_cell **cells, H2D_grid_cell_search_cell **cells_buffer, 
                                                     long *local_index_buffer, H2D_grid_cell_search_tile *parent, double center_lon, double center_lat, double dlon, double dlat, bool is_sphere_grid)
{
    this->num_cells = num_cells;
    this->cells = cells;
    this->cells_buffer = cells_buffer;
    this->local_index_buffer = local_index_buffer;
    this->parent = parent;
    this->center_lon = center_lon;
    this->center_lat = center_lat;
    this->dlon = dlon;
...
void H2D_grid_cell_search_tile::divide_tile()
{
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_cells >= 1 && cells[0]->get_bounding_circle_radius()> 0, "Software error in H2D_grid_cell_search_tile::divide_tile");
    
	cells_buffer[0] = cells[0];
	for (i = 1, j = 1; i < num_cells; i ++)
		if (cells[0]->get_center_lon() != cells[i]->get_center_lon() || cells[0]->get_center_lat() != cells[i]->get_center_lat())
			cells_buffer[j++] = cells[i];
	num_cells = j;
    for (i = 0; i < num_cells; i ++)
        cells[i] = cells_buffer[i];	

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_cells >= 1, "Software error in H2D_grid_cell_search_tile::divide_tile");

    if (num_cells <= MAX_NUM_CELLS_IN_TILE)
        return;

    seperate_cells_in_children_tiles(num_cells, cells, center_lon, center_lat, dlon, dlat, num_cells_in_children, local_index_buffer, is_sphere_grid);
    ...
    for (i = 0; i < num_cells; i ++)
        cells_buffer[displ_of_children_in_cells[local_index_buffer[i]]++] = cells[i];
    for (i = 0; i < num_cells; i ++)
        cells[i] = cells_buffer[i];
    for (i = 0, displ = 0; i < TILE_DIVIDE_FACTOR*TILE_DIVIDE_FACTOR; i ++) {
    ...
    for (i = 0, j = 0; i < TILE_DIVIDE_FACTOR*TILE_DIVIDE_FACTOR; i ++)
        if (num_cells_in_children[i] != 0) {
            children[j++] = new H2D_grid_cell_search_tile(num_cells_in_children[i], cells+displ_of_children_in_cells[i], cells_buffer, local_index_buffer,
                                                          this, children_center_lon[i], children_center_lat[i], new_dlon, new_dlat, is_sphere_grid);
        }
...
H2D_grid_cell_search_engine::H2D_grid_cell_search_engine(Remap_grid_class *remap_grid, const double *center_lons, const double *center_lats, const bool *masks, 
                                                         int num_vertex, const double *vertex_lons, const double *vertex_lats, int edge_type, bool build_search_structure)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, remap_grid->get_is_H2D_grid(), "Software error1 in H2D_grid_cell_search_engine::H2D_grid_cell_search_engine");
	EXECUTION_REPORT(REPORT_ERROR, -1, words_are_the_same(remap_grid->get_a_leaf_grid(COORD_LABEL_LON)->get_coord_unit(), COORD_UNIT_DEGREES), "Software error1 in H2D_grid_cell_search_engine::H2D_grid_cell_search_engine");
    
    this->remap_grid = remap_grid;
	this->grid_min_lon = remap_grid->get_boundary_min_lon();
	this->grid_max_lon = remap_grid->get_boundary_max_lon();
	this->grid_min_lat = remap_grid->get_boundary_min_lat();
	this->grid_max_lat = remap_grid->get_boundary_max_lat();
	this->grid_lat_range = grid_max_lat - grid_min_lat;
	this->is_sphere_grid = words_are_the_same(remap_grid->get_a_leaf_grid(COORD_LABEL_LON)->get_coord_unit(), COORD_UNIT_DEGREES);
	if (grid_min_lon > grid_max_lon) {
		EXECUTION_REPORT(REPORT_ERROR, -1, is_sphere_grid, "Software error1 in H2D_grid_cell_search_engine::H2D_grid_cell_search_engine");
		grid_lon_range = grid_max_lon + 360.0 - grid_min_lon;
	}
	else grid_lon_range = grid_max_lon - grid_min_lon;
    ...
    local_index_buffer = new long [num_cells];
	global_index_buffer = new long [num_cells];
    dist_buffer = new double [num_cells];
	if (is_sphere_grid)
	    dist_threshold = 1 / 6000.0;
	else dist_threshold = sqrt(10*(grid_lon_range*grid_lat_range)/num_cells);
	
    for (int i = 0, num_cells = 0; i < remap_grid->get_grid_size(); i ++) {
        if (masks != NULL)
            mask = masks[i];
        cells[i] = new H2D_grid_cell_search_cell(i, center_lons[i], center_lats[i], mask, num_vertex, vertex_lons+num_vertex*i, vertex_lats+num_vertex*i, edge_type, is_sphere_grid);
	...
	if (report_error_enabled)
		remap_grid->check_coord_values_range();

	if (is_sphere_grid)
	    recursively_search_initial_boundary(180, 0, 360, 180, center_lon, center_lat, dlon, dlat);
	else recursively_search_initial_boundary((grid_min_lon+grid_max_lon)/2, (grid_min_lat+grid_max_lat)/2, grid_lon_range, grid_lat_range, center_lon, center_lat, dlon, dlat);
    ...
    if (build_search_structure)
        root_tile = new H2D_grid_cell_search_tile(num_cells, cells_ptr, cells_buffer, local_index_buffer, NULL, center_lon, center_lat, dlon, dlat, is_sphere_grid);
    else root_tile = NULL;
}
                                                                      ...
                                                                      double &result_center_lat, double &result_dlon, double &result_dlat)
{
    ...
    int i, num_children = 0, unique_child_indx, indx_at_lon, indx_at_lat;
    double new_dlon, new_dlat, min_lon, min_lat, new_center_lon, new_center_lat;
	bool found_target = false;


	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, num_cells > 0, "Software error in H2D_grid_cell_search_engine::recursively_search_initial_boundary");
    seperate_cells_in_children_tiles(num_cells, cells_ptr, center_lon, center_lat, dlon, dlat, num_cells_in_children, local_index_buffer, is_sphere_grid);
	...
	if (num_cells == 1) {
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, cells_ptr[0]->get_bounding_circle_radius() > 0, "Software error in H2D_grid_cell_search_engine::recursively_search_initial_boundary");
		double dist = calculate_distance_of_two_points_2D(center_lon-dlon/2, center_lat, center_lon, center_lat, is_sphere_grid);
		if (dist <= cells_ptr[0]->get_bounding_circle_radius() * TILE_DIVIDE_FACTOR * 2) 
			found_target = true;
	}
    if (num_children > 1 || found_target) {
    ...
    if (new_center_lon >= 360 && is_sphere_grid)
        new_center_lon -= 360;
...
void H2D_grid_cell_search_engine::sort_nearest_points(int num_found_points)
{
	sort_grid_nearest_points(dist_buffer, local_index_buffer, global_index_buffer, num_found_points, remap_grid);
}


void H2D_grid_cell_search_engine::search_nearest_points_var_number(int num_required_points, long dst_cell_index, double dst_point_lon, double dst_point_lat, int &num_found_points, long *found_points_indx, double *found_points_dist, bool early_quit)
{
}


bool H2D_grid_cell_search_engine::search_nearest_points_var_distance(double dist_threshold, long dst_cell_index, double dst_point_lon, double dst_point_lat, int &num_found_points, long *found_points_indx, double *found_points_dist, bool early_quit)
{
    ...
    num_found_points = 0;
    have_the_same_point = root_tile->search_points_within_distance(dist_threshold, dst_point_lon, dst_point_lat, num_found_points, local_index_buffer, dist_buffer, early_quit);

	sort_nearest_points(num_found_points);
//    do_quick_sort(dist_buffer, local_index_buffer, 0, num_found_points-1);
    ...
    if (have_the_same_point && early_quit) {
        ...
        found_points_indx[0] = local_index_buffer[0];
        found_points_dist[0] = dist_buffer[0];
        return have_the_same_point;
    }

    for (int i = 0; i < num_found_points; i ++) {
        found_points_indx[i] = local_index_buffer[i];
...
void H2D_grid_cell_search_engine::search_overlapping_cells(int &num_overlapping_cells, long *overlapping_cells_index, const H2D_grid_cell_search_cell *dst_cell, bool accurately_match, bool early_quit) const
{
    ...
    root_tile->search_overlapping_cells(num_overlapping_cells, local_index_buffer, dst_cell, accurately_match, early_quit);
    ...
    for (int i = 0; i < num_overlapping_cells; i ++)
        overlapping_cells_index[i] = local_index_buffer[i];
...
/*
    ...
    for (int i = 0; i < remap_grid->get_grid_size(); i ++) {
        ...
        if (result1 != result2) {
            EXECUTION_REPORT_LOG(REPORT_ERROR, -1, false, "error1 in H2D_grid_cell_search_engine::search_overlapping_cells for %d %d\n", dst_cell->get_cell_index(), cells[i]->get_cell_index());
        }
        if (cells[i]->check_overlapping(dst_cell, accurately_match)) {
            local_index_buffer[temp_num_overlapping_cells++] = cells[i]->get_cell_index();
        }
    }
    do_quick_sort(local_index_buffer, (long*) NULL, 0, temp_num_overlapping_cells-1);
    
    bool same_result = temp_num_overlapping_cells == num_overlapping_cells;
    if (same_result)
        for (int i = 0; i < num_overlapping_cells; i ++)
            if (local_index_buffer[i] != overlapping_cells_index[i]) {
                same_result = false;
                break;
            }
    if (!same_result) {
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "error in H2D_grid_cell_search_engine::search_overlapping_cells\n");
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "result of all scan (%lf %lf): %d: ", dst_cell->get_center_lon(), dst_cell->get_center_lat(), temp_num_overlapping_cells);
        for (int i = 0; i < temp_num_overlapping_cells; i ++)
            EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "%d ", local_index_buffer[i]);
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "\n");
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "result of fast %d: ", num_overlapping_cells);
        for (int i = 0; i < num_overlapping_cells; i ++)
            EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "%d ", overlapping_cells_index[i]);
        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "\n");
...
int H2D_grid_cell_search_engine::search_cell_of_locating_point(double point_lon, double point_lat, bool accurately_match) const
{
    ...
    temp_cell = new H2D_grid_cell_search_cell(0, point_lon, point_lat, true, 0, NULL, NULL, EDGE_TYPE_LATLON, is_sphere_grid);

    search_overlapping_cells(num_overlapping_cells, local_index_buffer, temp_cell, accurately_match, true);

    if (num_overlapping_cells == 0)
        return -1;
    else return local_index_buffer[0];
