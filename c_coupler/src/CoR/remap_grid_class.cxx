...
void Remap_grid_class::initialize_grid_class_data()
{
	...
	this->local_cell_global_indexes = NULL;
}
...
Remap_grid_class::Remap_grid_class(const char *grid_name, const char *whole_grid_name)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, false, "\"%s\" is a partial grid or has partial sub grids. It is not supported in C-Coupler\n", grid_name);
...
Remap_grid_class::Remap_grid_class(const char *grid_name, const char *coord_unit, Remap_grid_data_class *center_lons, Remap_grid_data_class *center_lats, Remap_grid_data_class *vertex_lons, Remap_grid_data_class *vertex_lats, Remap_grid_data_class *grid_mask, Remap_grid_data_class *grid_area, int num_vertexes)
{
	char lon_grid_name[NAME_STR_SIZE], lat_grid_name[NAME_STR_SIZE];

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, center_lons != NULL, "Software error in Remap_grid_class::Remap_grid_class for direct decomp grid");
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, center_lats != NULL, "Software error in Remap_grid_class::Remap_grid_class for direct decomp grid");
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, grid_mask != NULL, "Software error in Remap_grid_class::Remap_grid_class for direct decomp grid");

	initialize_grid_class_data();
    sprintf(lon_grid_name, "lon_%s", grid_name);
    sprintf(lat_grid_name, "lat_%s", grid_name);
    Remap_grid_class *CoR_lon_grid = new Remap_grid_class(lon_grid_name, "lon", coord_unit, "acyclic", 0);
    Remap_grid_class *CoR_lat_grid = new Remap_grid_class(lat_grid_name, "lat", coord_unit, "acyclic", 0);	
	sub_grids.push_back(CoR_lon_grid);
	sub_grids.push_back(CoR_lat_grid);
	this->grid_size = center_lats->get_grid_data_field()->required_data_size;
	this->num_vertexes = num_vertexes;
	strcpy(this->grid_name, grid_name);
	this->num_dimensions = 2;
	this->masks_are_known = true;
	CoR_lon_grid->set_super_grid_of_setting_coord_values(this);
	CoR_lat_grid->set_super_grid_of_setting_coord_values(this);
	CoR_lon_grid->check_and_set_first_super_grid_of_enable_setting_coord_value(this);
	CoR_lat_grid->check_and_set_first_super_grid_of_enable_setting_coord_value(this);
	grid_center_fields.push_back(center_lons);
	grid_center_fields.push_back(center_lats);
	grid_mask_field = grid_mask;
	if (vertex_lons != NULL) {
		grid_vertex_fields.push_back(vertex_lons);
		grid_vertex_fields.push_back(vertex_lats);
	}
	imported_area = grid_area;
	
	center_lons->reset_coord_value_grid(this);
	center_lats->reset_coord_value_grid(this);
	if (vertex_lons != NULL)
		vertex_lons->reset_coord_value_grid(this);
	if (vertex_lats != NULL)
		vertex_lats->reset_coord_value_grid(this);
	if (imported_area != NULL)
		imported_area->reset_coord_value_grid(this);
	if (grid_mask_field != NULL)
		grid_mask_field->reset_coord_value_grid(this);
	generated_from_duplication = true;
}
...
Remap_grid_class::~Remap_grid_class()
{
	...
	if (local_cell_global_indexes != NULL)
		delete [] local_cell_global_indexes;
}


bool Remap_grid_class::get_grid_cyclic()
{
	if (num_dimensions == 1) {
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, words_are_the_same(coord_label, COORD_LABEL_LON), "Software error in Remap_grid_class::get_grid_cyclic");
		return cyclic; 
	}

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, get_is_H2D_grid() && words_are_the_same(sub_grids[0]->coord_label, COORD_LABEL_LON), "Software error in Remap_grid_class::get_grid_cyclic");
	return sub_grids[0]->cyclic;
}
...
/*
    EXECUTION_REPORT(REPORT_ERROR, -1, num_dimensions == 1 && super_grid->get_grid_size() > 0, 
        "remap software error: only one-dimension grid can check and set super grid of setting coordinate value size: %d\n", super_grid->get_grid_size());
*/
    EXECUTION_REPORT(REPORT_ERROR, -1, num_dimensions == 1, 
...
Remap_grid_class *Remap_grid_class::duplicate_grid(Remap_grid_class *top_grid)
{
    ...
    if (this->num_dimensions == 1 && this->super_grid_of_setting_coord_values != NULL && 
		...
		for (i = 0; i < remap_grid_manager->remap_grids.size(); i ++)
			if (remap_grid_manager->remap_grids[i] == top_grid)
				break;
		if (i == remap_grid_manager->remap_grids.size())
			similar_grid = this->super_grid_of_setting_coord_values;
		else {
	        for (i = 0; i < remap_grid_manager->remap_grids.size(); i ++) 
	...
	if (grid_mask_field != NULL)
		duplicated_grid->grid_mask_field = grid_mask_field->duplicate_grid_data_field(duplicated_grid, 1, true, true);
	if (area_or_volumn != NULL) {
		duplicated_grid->area_or_volumn = new double [grid_size];
		memcpy(duplicated_grid->area_or_volumn, area_or_volumn, grid_size*sizeof(double));
	}
	if (imported_area != NULL)
		duplicated_grid->imported_area = imported_area->duplicate_grid_data_field(duplicated_grid, 1, true, true);		
...

{
    ...
    if (runtime_mask != NULL) {
        if (runtime_mask->get_coord_value_grid()->is_similar_grid_with(remap_grid)) {
			if (runtime_remap_grid->grid_mask_field != NULL)
				delete runtime_remap_grid->grid_mask_field;
...
void Remap_grid_class::compute_ocn_mask(const char *topo_field_name, double unit_trans)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, topo_grid->get_is_H2D_grid(),
                                                        ...
                                                        double scale_factor)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, field_lev_coord_bot->get_coord_value_grid()->is_subset_of_grid(this) && field_lev_coord_bot->get_coord_value_grid()->get_is_H2D_grid(),
...
void Remap_grid_class::read_grid_data_from_array(const char *coord_label, const char *coord_name, const char *data_type, const char *array_data, int num_vertexes)
{
    ...
    if (words_are_the_same(coord_label, GRID_MASK_LABEL)) {
        ...
        if (words_are_the_same(coord_label, GRID_VERTEX_LABEL)) {
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, get_grid_center_field(coord_name) != NULL, "Software error in Remap_grid_class::read_grid_data_from_array");
			if (array_data != NULL) 
				check_vertex_coord_value_can_be_set(coord_name);
			else if (get_grid_center_field(coord_name)->get_coord_value_grid() != this) {
				get_grid_center_field(coord_name)->get_coord_value_grid()->read_grid_data_from_array(coord_label, coord_name, data_type, array_data, num_vertexes);
				return;
			}
		}
	...
	if (remap_data_field->read_data_size > 0 && array_data != NULL)
		...
		remap_data_field->data_buf = NULL;
		remap_data_field->required_data_size = 0;
		remap_data_field->read_data_size = 0;
	}
    if (words_are_the_same(coord_label, GRID_VERTEX_LABEL))
	...
	int array_size = array_data != NULL? grid_size : 0; 
    if (words_are_the_same(coord_label, GRID_MASK_LABEL)) {
        EXECUTION_REPORT(REPORT_ERROR, -1, words_are_the_same(data_type, DATA_TYPE_INT) || words_are_the_same(data_type, DATA_TYPE_BOOL), "Software error in Remap_grid_class::read_grid_data_from_array: wrong data type for mask");
		if (words_are_the_same(data_type, DATA_TYPE_INT)) {
			...
			for (int i = 0; i < array_size; i ++)
            	((bool*)remap_data_field->data_buf)[i] = ((bool*) array_data)[i];
		}
    }
    else {
        if (num_vertexes > 0)
                                              ...
                                              int num_vertexes)
{
    ...
    if ((words_are_the_same(extension_names[0], GRID_CENTER_LABEL) || words_are_the_same(extension_names[0], GRID_VERTEX_LABEL))) {
        if (words_are_the_same(extension_names[1], COORD_LABEL_LON) || words_are_the_same(extension_names[1], COORD_LABEL_LAT)) {
            ...
            if (num_dimensions == 2)
                EXECUTION_REPORT(REPORT_ERROR, -1, this->get_is_H2D_grid(), "the 2D grid %s to be set lon or lat values must be a sphere grid with coordinates lon, and lat\n",
                             this->grid_name);
        }
...
Remap_grid_data_class *Remap_grid_class::get_grid_center_field(const char *label) const
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, false, "Software error in Remap_grid_class::get_grid_center_field: fail to find a leaf grid with coordinate \"%s\"", label);
...
Remap_grid_data_class *Remap_grid_class::get_grid_vertex_field(const char *label) const
{
    int num_leaf_grids;
    Remap_grid_class *leaf_grids[256];
    ...
    for (int i = 0; i < num_leaf_grids; i ++)
        if (words_are_the_same(leaf_grids[i]->get_coord_label(), label))
            return leaf_grids[i]->get_grid_vertex_field();

    EXECUTION_REPORT(REPORT_ERROR, -1, false, "Software error in Remap_grid_class::get_grid_vertex_field: fail to find a leaf grid with coordinate \"%s\"", label);
                                                             ...
                                                             bool is_unit_degree_dim2)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, get_is_H2D_grid(), "grid \"%s\" does not match the constraint that the grid which can be set 2D vertex values must be 2D sphere grid", grid_name);
...
void Remap_grid_class::set_grid_boundary(double min_lon, double max_lon, double min_lat, double max_lat)
{
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, this->get_is_H2D_grid(), "%s is not a sphere grid, while only sphere grid can be set boundary", grid_name);
    EXECUTION_REPORT(REPORT_ERROR, -1, boundary_min_lon == NULL_COORD_VALUE, "the boundary of grid %s has been set before", grid_name);
	if (this->get_is_sphere_grid() && words_are_the_same(get_sphere_grid_coord_unit(), COORD_UNIT_DEGREES) || words_are_the_same(get_sphere_grid_coord_unit(), COORD_UNIT_RADIANS)) {
	    ...
	    if (fabs(min_lat+90) < 0.00001)
	        min_lat = -90;
	    if (fabs(max_lat-90) < 0.00001)
	        max_lat = 90;
		while (min_lon < 0)
			min_lon += 360.0;
		while (min_lon > 360.0)
...
void Remap_grid_class::check_coord_values_range() const
{
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, super_grid_of_setting_coord_values->boundary_min_lon != NULL_COORD_VALUE, "Software error in Remap_grid_class::check_coord_values_range : %lx  %s  %s", this, grid_name, super_grid_of_setting_coord_values->get_grid_name());
    if (words_are_the_same(coord_label, COORD_LABEL_LON)) {
        range_start = super_grid_of_setting_coord_values->boundary_min_lon;
        range_end = super_grid_of_setting_coord_values->boundary_max_lon;
    }
    else if (words_are_the_same(coord_label, COORD_LABEL_LAT)){
        range_start = super_grid_of_setting_coord_values->boundary_min_lat;
        range_end = super_grid_of_setting_coord_values->boundary_max_lat;
    }
    ...
    for (i = 0; i < center_field->grid_data_field->required_data_size; i ++) {
		...
		if (range_start < range_end) {
	        EXECUTION_REPORT(REPORT_ERROR, -1, ((double*) center_field->grid_data_field->data_buf)[i] >= range_start &&
    	                   ((double*) center_field->grid_data_field->data_buf)[i] <= range_end,
        	               "the center values of coordinate \"%s\" of grid \"%s\" must be between %s (currently is %lf) \n",
            	           coord_label, super_grid_of_setting_coord_values->grid_name, tmp_string, ((double*) center_field->grid_data_field->data_buf)[i]);
		}
		else EXECUTION_REPORT(REPORT_ERROR, -1, ((double*) center_field->grid_data_field->data_buf)[i] <= range_start &&
    	                   ((double*) center_field->grid_data_field->data_buf)[i] >= range_end,
        	               "the center values of coordinate \"%s\" of grid \"%s\" must be between %s: (currently is %lf) \n",
            	           coord_label, super_grid_of_setting_coord_values->grid_name, tmp_string, ((double*) center_field->grid_data_field->data_buf)[i]);
    }
    if (vertex_field != NULL)
        for (i = 0; i < vertex_field->grid_data_field->required_data_size; i ++) 
            if (((double*) vertex_field->grid_data_field->data_buf)[i] != NULL_COORD_VALUE) {
				if (range_start < range_end) {
	                EXECUTION_REPORT(REPORT_ERROR, -1, ((double*) vertex_field->grid_data_field->data_buf)[i] >= range_start &&
    	                           ((double*) vertex_field->grid_data_field->data_buf)[i] <= range_end,
        	                       "the vertex values (currently include %lf) of coordinate \"%s\" of grid \"%s\" must be between %s\n",
            	                   ((double*) vertex_field->grid_data_field->data_buf)[i], coord_label, super_grid_of_setting_coord_values->grid_name, tmp_string);
				}
				else EXECUTION_REPORT(REPORT_ERROR, -1, ((double*) vertex_field->grid_data_field->data_buf)[i] <= range_start &&
    	                           ((double*) vertex_field->grid_data_field->data_buf)[i] >= range_end,
...
void Remap_grid_class::check_center_vertex_values_consistency_2D(bool force)
{
    ...
    if (!force && (are_vertex_values_set_in_default || grid_center_fields.size() != 2 || grid_vertex_fields.size() == 0 || grid_center_fields[0]->grid_data_field->required_data_size == 0))
        return;
    
    EXECUTION_REPORT(REPORT_ERROR, -1, grid_center_fields.size() == grid_vertex_fields.size(), "remap software error1 in check_center_vertex_values_relation_2D: %d vs %d\n", grid_center_fields.size(), grid_vertex_fields.size());
    for (i = 0; i < grid_center_fields.size(); i ++) {
...
void Remap_grid_class::formalize_sphere_grid()
{
    ...
    if (!get_is_H2D_grid() || grid_center_fields.size() < 2)
        return;
...
void Remap_grid_class::end_grid_definition_stage(Remap_operator_basis *remap_operator)
{
    ...
    for (int i = 0; i < remap_grid_manager->remap_grids.size(); i ++)
        if (remap_grid_manager->remap_grids[i]->is_subset_of_grid(this)) {
            ...
            remap_grid_manager->remap_grids[i]->check_center_vertex_values_consistency_2D(false);
...
void Remap_grid_class::formalize_cyclic_coord_values(Remap_grid_data_class *remap_grid_data)
{
    ...
    for (i = 0; i < num_leaf_grids; i ++)
        if (words_are_the_same(leaf_grids[i]->coord_label, remap_grid_data->grid_data_field->field_name_in_application))
            break;
    if (!words_are_the_same(leaf_grids[i]->coord_unit, COORD_UNIT_DEGREES) || !words_are_the_same(leaf_grids[i]->coord_label, COORD_LABEL_LON))
        return;
...
void Remap_grid_class::calculate_area_of_sphere_grid() 
{
	...
	if (get_grid_vertex_field(COORD_LABEL_LON)->get_grid_data_field()->read_data_size == 0)
		return;
    ...
    for (i = 0; i < this->grid_size; i ++) {
//        EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "the %d point in remap_grid_class", i);
        if (tmp_num_vertex > 2) {
            should_rotate = false;
            ...
            for (j = 0; j < tmp_num_vertex; j ++) {
                ...
                if (fabs(current_lat_vertex_values[current_num_vertex]) > SPHERE_GRID_ROTATION_LAT_THRESHOLD)
                    should_rotate = true;
//                EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "the vertex point in remap_grid_class %lf %lf", current_lon_vertex_values[current_num_vertex], current_lat_vertex_values[current_num_vertex]);
                current_num_vertex ++;
            }
        }
        else {
...
bool Remap_grid_class::get_is_H2D_grid() const
{
    return this->num_dimensions == 2 && this->has_grid_coord_label(COORD_LABEL_LON) && this->has_grid_coord_label(COORD_LABEL_LAT);
}


const char *Remap_grid_class::get_coord_unit() const 
{ 
	if (get_is_H2D_grid())
		return sub_grids[0]->get_coord_unit();
...
/* There will be a bug for conservative remapping algorithm: vertexes are lost in decomp grid generation */
Remap_grid_class *Remap_grid_class::generate_decomp_grid(const int *local_cell_indexes, int num_local_cells, const char *decomp_name)
{
	...
	Remap_grid_data_class *original_vertex_field_lon = NULL, *original_vertex_field_lat = NULL;
    ...
    double *decomp_lon_vertex_values, *decomp_lat_vertex_values;
    double *this_lon_vertex_values, *this_lat_vertex_values;
    bool *decomp_mask_values, *this_mask_values;
	bool has_original_2D_coord_values;
    int i, j;
    ...
    EXECUTION_REPORT(REPORT_ERROR, -1, leaf_grids[0]->get_grid_center_field() != NULL && original_center_field_lat != NULL,
                 "user must specify the center coordinate of the horizontal 2D grid %s\n", this->grid_name);

    decomp_grid->grid_center_fields.push_back(original_center_field_lon->duplicate_grid_data_field(decomp_grid, 1, false, false));
    decomp_grid->grid_center_fields.push_back(original_center_field_lat->duplicate_grid_data_field(decomp_grid, 1, false, false));
	...
	if (this->are_all_vertex_fields_specified_by_user()) {
		if (this->num_vertexes > 0)
			decomp_grid->num_vertexes = this->num_vertexes;
		else {
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, leaf_grids[0]->grid_vertex_fields.size() == 1 && leaf_grids[0]->are_all_vertex_fields_specified_by_user() && leaf_grids[0]->num_vertexes == 2, "Software error in Remap_grid_class::generate_decomp_grid");
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, leaf_grids[1]->grid_vertex_fields.size() == 1 && leaf_grids[1]->are_all_vertex_fields_specified_by_user() && leaf_grids[1]->num_vertexes == 2, "Software error in Remap_grid_class::generate_decomp_grid");
			decomp_grid->num_vertexes = 4;
		}
	}
	else decomp_grid->num_vertexes = this->num_vertexes;

	if (decomp_grid->num_vertexes > 0 || leaf_grids[0]->get_grid_vertex_field() != NULL)
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, decomp_grid->num_vertexes > 0 && leaf_grids[0]->get_grid_vertex_field() != NULL, "Software error in Remap_grid_class *Remap_grid_class::generate_decomp_grid");

	if (leaf_grids[0]->get_grid_vertex_field() != NULL) {
		decomp_grid->grid_vertex_fields.push_back(leaf_grids[0]->get_grid_vertex_field()->duplicate_grid_data_field(decomp_grid, decomp_grid->num_vertexes, false, false));
		decomp_grid->grid_vertex_fields.push_back(leaf_grids[1]->get_grid_vertex_field()->duplicate_grid_data_field(decomp_grid, decomp_grid->num_vertexes, false, false));		
	}

    if (area_or_volumn != NULL)
        decomp_grid->area_or_volumn = new double [num_local_cells];
	if (leaf_grids[0]->get_grid_vertex_field() != NULL) {
		original_vertex_field_lon = leaf_grids[0]->get_grid_vertex_field();
		original_vertex_field_lat = leaf_grids[1]->get_grid_vertex_field();
	}
    if (original_center_field_lon->get_coord_value_grid()->get_num_dimensions() == 2) {
		has_original_2D_coord_values = true;
		if (original_vertex_field_lon != NULL && original_vertex_field_lon->get_grid_data_field()->data_buf != NULL) {
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, original_vertex_field_lon->get_coord_value_grid()->get_num_dimensions() == 2, "C-Coupler error in generate_decomp_grid\n");			
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, original_vertex_field_lon->get_grid_data_field()->required_data_size == this->grid_size * this->get_num_vertexes(), "Software error in generate_decomp_grid: %d  vs  %d\n", original_vertex_field_lon->get_grid_data_field()->required_data_size, this->grid_size * this->get_num_vertexes());
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, original_vertex_field_lat->get_grid_data_field()->required_data_size == this->grid_size * this->get_num_vertexes(), "Software error in generate_decomp_grid\n");
		}
    }
    else {
		has_original_2D_coord_values = false;
		if (original_vertex_field_lon != NULL && original_vertex_field_lon->get_grid_data_field()->data_buf != NULL)
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, original_vertex_field_lon->get_coord_value_grid()->get_num_dimensions() == 1, "C-Coupler error in generate_decomp_grid\n");
    }
	...
	if (original_vertex_field_lon != NULL && original_vertex_field_lon->get_grid_data_field()->data_buf != NULL) {
	    decomp_lon_vertex_values = (double*) decomp_grid->grid_vertex_fields[0]->get_grid_data_field()->data_buf;
	    decomp_lat_vertex_values = (double*) decomp_grid->grid_vertex_fields[1]->get_grid_data_field()->data_buf;
    	decomp_mask_values = (bool*) decomp_grid->grid_mask_field->get_grid_data_field()->data_buf;
	    this_lon_vertex_values = (double*) original_vertex_field_lon->get_grid_data_field()->data_buf;
    	this_lat_vertex_values = (double*) original_vertex_field_lat->get_grid_data_field()->data_buf;
	}
	if (this_lon_center_values != NULL) {
	    for (i = 0; i < num_local_cells; i ++) {
	        if (local_cell_indexes[i] != CCPL_NULL_INT) {
				EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, local_cell_indexes[i] >= 0 && local_cell_indexes[i] < this->grid_size, "Software error in Remap_grid_class::generate_decomp_grid");
				...
				if (has_original_2D_coord_values) {
					...
					if (original_vertex_field_lon != NULL && original_vertex_field_lon->get_grid_data_field()->data_buf != NULL) {
						for (int j = 0; j < this->num_vertexes; j ++) {
							decomp_lon_vertex_values[i*this->num_vertexes+j] = this_lon_vertex_values[local_cell_indexes[i]*this->num_vertexes+j];
							decomp_lat_vertex_values[i*this->num_vertexes+j] = this_lat_vertex_values[local_cell_indexes[i]*this->num_vertexes+j];
						}
					}
				}
				if (!has_original_2D_coord_values) {
					int temp_i = local_cell_indexes[i]%leaf_grids[0]->get_grid_size();
					int temp_j = local_cell_indexes[i]/leaf_grids[0]->get_grid_size();
					decomp_lon_center_values[i] = this_lon_center_values[temp_i];
					decomp_lat_center_values[i] = this_lat_center_values[temp_j];
					if (original_vertex_field_lon != NULL && original_vertex_field_lon->get_grid_data_field()->data_buf != NULL) {
						decomp_lon_vertex_values[i*4+0] = this_lon_vertex_values[temp_i*2];
						decomp_lon_vertex_values[i*4+1] = this_lon_vertex_values[temp_i*2];
						decomp_lon_vertex_values[i*4+2] = this_lon_vertex_values[temp_i*2+1];
						decomp_lon_vertex_values[i*4+3] = this_lon_vertex_values[temp_i*2+1];
						decomp_lat_vertex_values[i*4+0] = this_lat_vertex_values[temp_j*2];
						decomp_lat_vertex_values[i*4+1] = this_lat_vertex_values[temp_j*2+1];
						decomp_lat_vertex_values[i*4+2] = this_lat_vertex_values[temp_j*2+1];
						decomp_lat_vertex_values[i*4+3] = this_lat_vertex_values[temp_j*2];
					}
				}
	        }
	        else {
				...
				if (original_vertex_field_lon != NULL && original_vertex_field_lon->get_grid_data_field()->data_buf != NULL) {
					for (int j = 0; j < this->num_vertexes; j ++) {
			            decomp_lon_vertex_values[i*this->num_vertexes+j] = NULL_COORD_VALUE;
			            decomp_lat_vertex_values[i*this->num_vertexes+j] = NULL_COORD_VALUE;
					}
				}
	        }
	    }
	}
	else decomp_grid->empty_grid_data();
...
void Remap_grid_class::write_grid_into_array(char **array, long &buffer_max_size, long &buffer_content_size)
{
    ...
    write_grid_field_into_array(imported_area, array, buffer_max_size, buffer_content_size);
    ...
    write_data_into_array_buffer(&temp_int, sizeof(int), array, buffer_max_size, buffer_content_size);
	if (local_cell_global_indexes != NULL)
		write_data_into_array_buffer(local_cell_global_indexes, sizeof(int)*grid_size, array, buffer_max_size, buffer_content_size);
	temp_int = local_cell_global_indexes == NULL? 0 : 1;
	write_data_into_array_buffer(&temp_int, sizeof(int), array, buffer_max_size, buffer_content_size);
...
Remap_grid_class::Remap_grid_class(Remap_grid_class *top_grid, const char *grid_name_suffix, const char *array, long &buffer_content_iter, bool is_global_grid)
{
    int temp_int;
    char temp_grid_name[NAME_STR_SIZE];
	...
	if (temp_int == 1) {
		if (grid_size > 0)
			local_cell_global_indexes = new int [grid_size];
		read_data_from_array_buffer(local_cell_global_indexes, sizeof(int)*grid_size, array, buffer_content_iter, true);
	}
    read_data_from_array_buffer(&temp_int, sizeof(int), array, buffer_content_iter, true);
    ...
    read_grid_field_from_array(&imported_area, array, buffer_content_iter);
    ...
    read_data_from_array_buffer(&temp_int, sizeof(int), array, buffer_content_iter, true);
    for (int i = 0; i < temp_int; i ++) {
        Remap_grid_class *child_grid = new Remap_grid_class(top_grid, grid_name_suffix, array, buffer_content_iter, is_global_grid);
		if (is_global_grid) {
			...
			sub_grids.push_back(child_grid);
			generated_from_duplication = true;
		}
    }
...
const char *Remap_grid_class::get_sphere_grid_coord_unit()
{
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, get_is_H2D_grid(), "Software error in Remap_grid_class::get_sphere_grid_coord_unit");
	return sub_grids[0]->get_coord_unit();
}
...
void Remap_grid_class::set_local_cell_global_indexes(const int *local_cell_global_indexes, bool minus_1)
{
	if (grid_size == 0)
		return;

	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, this->local_cell_global_indexes == NULL, "Software error in Remap_grid_class::set_local_cell_global_indexes");

	this->local_cell_global_indexes = new int [grid_size];
	for (int i = 0; i < grid_size; i ++) {
		this->local_cell_global_indexes[i] = local_cell_global_indexes[i];
		if (minus_1 && this->local_cell_global_indexes[i] != CCPL_NULL_INT)
			this->local_cell_global_indexes[i] --;
	}
}


int Remap_grid_class::get_num_active_cells()
{
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, grid_mask_field->get_grid_data_field()->required_data_size == grid_size, "Software error in Remap_grid_class::get_num_active_cells");
	mask = (bool*)(grid_mask_field->get_grid_data_field()->data_buf);
	for (int i = 0; i < grid_size; i ++)
		if (mask[i])
			num_active_cells ++;
...
bool Remap_grid_class::is_point_in_subdomain(double point_lon, double point_lat, double subdomain_min_lon, double subdomain_max_lon, double subdomain_min_lat, double subdomain_max_lat)
{
	if (!(point_lat >= subdomain_min_lat && point_lat <= subdomain_max_lat))
		return false;

	if (subdomain_min_lon <= subdomain_max_lon)
		return point_lon >= subdomain_min_lon && point_lon <= subdomain_max_lon;

	return point_lon >= subdomain_min_lon || point_lon <= subdomain_max_lon;
}


bool Remap_grid_class::are_all_active_cells_in_subdomain(double subdomain_min_lon, double subdomain_max_lon, double subdomain_min_lat, double subdomain_max_lat)
{
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, grid_size > 0 && grid_vertex_fields.size() > 0, "Software error in Remap_grid_class::are_all_active_cells_in_subdomain");
	bool *mask = (bool*) grid_mask_field->get_grid_data_field()->data_buf;
	double *center_lons = (double*) grid_center_fields[0]->get_grid_data_field()->data_buf;
	double *center_lats = (double*) grid_center_fields[1]->get_grid_data_field()->data_buf;
	double *vertex_lons = (double*) grid_vertex_fields[0]->get_grid_data_field()->data_buf;
	double *vertex_lats = (double*) grid_vertex_fields[1]->get_grid_data_field()->data_buf;

	for (int i = 0; i < grid_size; i ++) {
		if (!is_point_in_subdomain(center_lons[i], center_lats[i], subdomain_min_lon, subdomain_max_lon, subdomain_min_lat, subdomain_max_lat))
			return false;
		for (int j = 0; j < num_vertexes; j ++)
			if (vertex_lons[i*num_vertexes+j] != NULL_COORD_VALUE && !is_point_in_subdomain(vertex_lons[i*num_vertexes+j], vertex_lats[i*num_vertexes+j],subdomain_min_lon, subdomain_max_lon, subdomain_min_lat, subdomain_max_lat))
				return false;
	}
...
void Remap_grid_class::empty_grid_data()
{
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, get_grid_center_field(COORD_LABEL_LON) != NULL, "Software error in Remap_grid_class::empty_grid_data");
	if (get_grid_center_field(COORD_LABEL_LON)->coord_value_grid != this) {
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, get_grid_center_field(COORD_LABEL_LON)->coord_value_grid != this, "Software error in Remap_grid_class::empty_grid_data");		
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, grid_center_fields.size() == 0, "Software error in Remap_grid_class::empty_grid_data");
		if (get_grid_vertex_field(COORD_LABEL_LON) != NULL) {
			grid_vertex_fields.push_back(get_grid_vertex_field(COORD_LABEL_LON));
			grid_vertex_fields.push_back(get_grid_vertex_field(COORD_LABEL_LAT));
			sub_grids[0]->grid_vertex_fields.clear();
			sub_grids[1]->grid_vertex_fields.clear();
			grid_vertex_fields[0]->coord_value_grid = this;
			grid_vertex_fields[1]->coord_value_grid = this;
		}
		grid_center_fields.push_back(get_grid_center_field(COORD_LABEL_LON));
		grid_center_fields.push_back(get_grid_center_field(COORD_LABEL_LAT));
		sub_grids[0]->grid_center_fields.clear();
		sub_grids[1]->grid_center_fields.clear();
		sub_grids[0]->super_grid_of_setting_coord_values = this;
		sub_grids[1]->super_grid_of_setting_coord_values = this;
		grid_center_fields[0]->coord_value_grid = this;
		grid_center_fields[1]->coord_value_grid = this;
	}
	EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "empty the data of grid %s  %lx", grid_name, this);
	for (int i = 0; i < grid_center_fields.size(); i ++)
		grid_center_fields[i]->empty_data();
	for (int i = 0; i < grid_vertex_fields.size(); i ++)
		grid_vertex_fields[i]->empty_data();

	if (grid_mask_field != NULL)
		grid_mask_field->empty_data();

	if (imported_area != NULL)
		imported_area->empty_data();
	...
	if (sigma_grid_sigma_value_field != NULL)
		sigma_grid_sigma_value_field->empty_data();

	if (hybrid_grid_coefficient_field != NULL)
		hybrid_grid_coefficient_field->empty_data();

	if (level_V3D_coord_trigger_field != NULL)
		level_V3D_coord_trigger_field->empty_data();

	if (local_cell_global_indexes != NULL) {
		delete [] local_cell_global_indexes;
		local_cell_global_indexes = NULL;
	}
}


void Remap_grid_class::remove_vertexes_info()
{
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, get_is_H2D_grid(), "Software error in Remap_grid_class::remove_vertexes_info");
	if (are_all_vertex_fields_specified_by_user())
		return;

	num_vertexes = 0;
	are_vertex_values_set_in_default = false;
	for (int i = 0; i < grid_vertex_fields.size(); i ++)
		delete grid_vertex_fields[i];
	grid_vertex_fields.clear();
}
