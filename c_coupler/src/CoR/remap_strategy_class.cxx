...
void Remap_strategy_class::calculate_remapping_weights(Remap_weight_of_strategy_class *remap_weight_of_strategy, const char *H2D_remapping_wgt_file, int wgt_cal_comp_id, int original_dst_decomp_id, int src_original_grid_id, int dst_original_grid_id)
{
    int i, j;
    ...
    for (i = 0; i < remap_operators.size(); i ++) {
        ...
        if (execution_phase_number == 1) {
            for (runtime_remap_times_iter = 0; runtime_remap_times_iter < current_remap_src_data_grid_interchanged->get_grid_size()/runtime_remap_grid_src->get_grid_size(); runtime_remap_times_iter ++) {
                current_runtime_remap_function->calculate_static_remapping_weights(runtime_remap_times_iter, H2D_remapping_wgt_file, wgt_cal_comp_id, original_dst_decomp_id, src_original_grid_id, dst_original_grid_id);
            }
        }
        if (remap_operators[i]->get_src_grid()->has_grid_coord_label(COORD_LABEL_LEV))
