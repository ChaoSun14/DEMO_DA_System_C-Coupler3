                                                               ...
                                                               long remap_beg_iter, Remap_operator_basis *remap_operator)
{
    ...
    if (remap_operator->get_src_grid()->get_is_H2D_grid()) {
...
Remap_weight_of_operator_instance_class *Remap_weight_of_operator_instance_class::generate_parallel_remap_weights(Remap_grid_class **decomp_original_grids)
{
    ...
    if (overlap_with_decomp_counter > 0) {
        ...
        parallel_remap_weights_of_operator_instance->duplicated_remap_operator = this->duplicated_remap_operator->duplicate_remap_operator(true);
    }
    else {
...
{
    EXECUTION_REPORT(REPORT_ERROR, -1, time_end_iter == -2 || remap_end_iter == time_end_iter, "C-Coupler error in Remap_weight_of_operator_instance_class::renew_remapping_time_end_iter");
...
{
    int i, j, k;
    Remap_weight_of_operator_instance_class *parallel_remap_weights_of_operator_instance = NULL;
	...
	Remap_operator_basis *original_remap_operator = NULL;
	...
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, remap_weights_of_operator_instances.size() > 0, "Software error in Remap_weight_of_operator_class::generate_parallel_remap_weights");
    ...
    for (i = 0; i < remap_weights_of_operator_instances.size(); i ++) {
        ...
        if (remap_weights_of_operator_instances[i]->remap_end_iter != -1)
			...
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, remap_weights_of_operator_instances.size() == 1, "Software error in Remap_weight_of_operator_class::generate_parallel_remap_weights");
		...
		EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, original_remap_operator == NULL || original_remap_operator == remap_weights_of_operator_instances[i]->get_original_remap_operator(), "Software error in Remap_weight_of_operator_class::generate_parallel_remap_weights");
		original_remap_operator = remap_weights_of_operator_instances[i]->get_original_remap_operator();
        if (remap_weights_of_operator_instances[i]->get_operator_grid_src()->get_is_H2D_grid()) {
            parallel_remap_weights_of_operator_instance = remap_weights_of_operator_instances[i]->generate_parallel_remap_weights(decomp_original_grids);
			...
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, remap_weights_of_operator_instances[i]->get_field_data_grid_src()->have_overlap_with_grid(decomp_original_grids[1]), "Software error in generate_parallel_remap_weights of Remap_weight_of_strategy_class\n");  // horizontal remapping is at the first constantly
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, remap_weights_of_operator_instances[i]->get_operator_grid_src()->get_num_dimensions() == 1, "Software error in Remap_weight_of_operator_class::generate_parallel_remap_weights");
			EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, words_are_the_same(remap_weights_of_operator_instances[i]->get_operator_grid_src()->get_coord_label(), COORD_LABEL_LEV), "Software error in Remap_weight_of_operator_class::generate_parallel_remap_weights: C-Coupler currently only support vertical 1-D remapping");
			Decomp_info *dst_decomp_info = decomps_info_mgr->get_decomp_info(dst_decomp_id);
			parallel_remap_weights_of_operator_instance = NULL;
            for (j = 0; j < dst_decomp_info->get_num_local_cells(); j ++) {
				if (dst_decomp_info->get_local_cell_global_indx()[j] == CCPL_NULL_INT) {
					parallel_remap_weights_of_operator_instance = NULL;
					continue;
				}
				if (parallel_remap_weights_of_operator_instance == NULL || remap_weights_of_operator_instances[i]->remap_end_iter == -1) {
	                parallel_remap_weights_of_operator_instance = remap_weights_of_operator_instances[i]->generate_parallel_remap_weights(decomp_original_grids);
	                field_data_grid_src = remap_related_decomp_grids[field_data_grids_iter+0];
	                field_data_grid_dst = remap_related_decomp_grids[field_data_grids_iter+1];
	                operator_grid_src = remap_related_decomp_grids[field_data_grids_iter+2];
	                operator_grid_dst = remap_related_decomp_grids[field_data_grids_iter+3];
	                parallel_remap_weights_of_operator_instance->remap_beg_iter = j;                
	                parallel_remap_weights_of_strategy->add_remap_weight_of_operator_instance(parallel_remap_weights_of_operator_instance, field_data_grid_src, field_data_grid_dst, parallel_remap_weights_of_operator_instance->get_original_remap_operator(), operator_grid_src, operator_grid_dst);
				}
				parallel_remap_weights_of_operator_instance->remap_end_iter = j+1; 
            }
        }
    }

	if (parallel_remap_weights_of_operator_instance == NULL) {
		parallel_remap_weights_of_operator_instance = remap_weights_of_operator_instances[0]->generate_parallel_remap_weights(decomp_original_grids);
		field_data_grid_src = remap_related_decomp_grids[field_data_grids_iter+0];
		field_data_grid_dst = remap_related_decomp_grids[field_data_grids_iter+1];
		operator_grid_src = remap_related_decomp_grids[field_data_grids_iter+2];
		operator_grid_dst = remap_related_decomp_grids[field_data_grids_iter+3];
		operator_grid_src->has_grid_coord_label(COORD_LABEL_LEV);
		operator_grid_dst->has_grid_coord_label(COORD_LABEL_LEV);
		parallel_remap_weights_of_operator_instance->remap_beg_iter = -1; 
                                                               ...
                                                               bool read_from_io)
{
    ...
    if (!read_from_io)
        remap_strategy->calculate_remapping_weights(this, NULL, -1, -1, -1, -1);
    else {
...
Remap_weight_of_strategy_class::Remap_weight_of_strategy_class(const char *object_name, Remap_strategy_class *remap_strategy, 
                                                               Remap_grid_class *data_grid_src, Remap_grid_class *data_grid_dst, const char *H2D_remapping_wgt_file, bool calculate_wgts, int wgt_cal_comp_id, int dst_decomp_id, int src_original_grid_id, int dst_original_grid_id)
{
    initialize_object();
    strcpy(this->object_name, object_name);
	...
	if (calculate_wgts) {
	    ...
	    remap_strategy->calculate_remapping_weights(this, H2D_remapping_wgt_file, wgt_cal_comp_id, dst_decomp_id, src_original_grid_id, dst_original_grid_id);
	}
}
...
Remap_weight_of_strategy_class::~Remap_weight_of_strategy_class()
{
	...
	for (int i = 0; i < num_field_data_grids_in_remapping_process; i ++) {
		if (field_data_grids_in_remapping_process[i] == NULL)
			continue;
		for (int j = i+1; j < num_field_data_grids_in_remapping_process; j ++)
			if (field_data_grids_in_remapping_process[i] == field_data_grids_in_remapping_process[j])
				field_data_grids_in_remapping_process[j] = NULL;
		if (remap_grid_manager->search_exact_grid(field_data_grids_in_remapping_process[i]) == NULL)
			delete field_data_grids_in_remapping_process[i];
	}
}
...
void Remap_weight_of_strategy_class::add_remap_weight_of_operator_instance(Remap_weight_of_operator_instance_class *weight_of_operator_instance, Remap_grid_class *field_data_grid_src, Remap_grid_class *field_data_grid_dst, Remap_operator_basis *original_remap_operator,
                                                                           Remap_grid_class *operator_grid_src, Remap_grid_class *operator_grid_dst) 
{ 
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, field_data_grid_src != NULL && field_data_grid_dst != NULL, "Software error in Remap_weight_of_strategy_class::add_remap_weight_of_operator_instance1");
	EXECUTION_REPORT_ERROR_OPTIONALLY(REPORT_ERROR, -1, operator_grid_src != NULL && operator_grid_dst != NULL, "Software error in Remap_weight_of_strategy_class::add_remap_weight_of_operator_instance");
...
{   
    ...
    for (int i = 0; i < remap_weights_of_operators.size(); i ++) {
		...
		add_remap_related_grid(all_remap_related_grids_info, remap_weights_of_operators[i]->get_field_data_grid_dst(), remap_weights_of_operators[i]->get_operator_grid_src()->get_is_H2D_grid() || after_H2D_remapping);
		...
		if (remap_weights_of_operators[i]->get_operator_grid_src()->get_is_H2D_grid())
                                                                                                 ...
                                                                                                 int dst_decomp_id)
{
    ...
    for (i = 0; i < remap_weights_of_operators.size(); i ++)
        remap_weights_of_operators[i]->generate_parallel_remap_weights(remap_related_decomp_grids, decomp_original_grids, dst_decomp_id, field_data_grids_iter, parallel_remap_weights_of_strategy);
