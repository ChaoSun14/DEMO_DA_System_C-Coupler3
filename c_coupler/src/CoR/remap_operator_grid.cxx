...
void Remap_operator_grid::update_operator_grid_data()
{
    ...
    if (remap_operator->get_num_dimensions() > 1 && remap_operator->get_is_operator_regridding()) {
        if (grid2D_search_engine == NULL)
            grid2D_search_engine = new H2D_grid_cell_search_engine(remap_grid, center_coord_values[0], center_coord_values[1], mask_values,
...
void Remap_operator_grid::initialize_for_vertex_coord_values_generation()
{
    ...
    if (remap_grid->get_is_H2D_grid() && !remap_grid->check_vertex_fields_completeness(remap_operator))
        EXECUTION_REPORT(REPORT_ERROR, -1, false, "Software error in Remap_operator_grid::initialize_for_vertex_coord_values_generation");
