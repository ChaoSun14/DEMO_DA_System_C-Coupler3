...
//    EXECUTION_REPORT(REPORT_ERROR, -1, search_remap_grid_with_grid_name(remap_grid->get_grid_name()) == NULL, "Software error in Remap_grid_mgt::add_remap_grid: grid \"%s\" has already been added into the grid manager", remap_grid->get_grid_name());
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "add remap grid \"%s\"", remap_grid->get_grid_name());
...
void Remap_grid_mgt::add_temp_grid(Remap_grid_class *temp_grid)
{
    ...
    EXECUTION_REPORT_LOG(REPORT_LOG, -1, true, "add temp grid \"%s\"", temp_grid->get_grid_name());
}
...
Remap_grid_class *Remap_grid_mgt::search_exact_grid(Remap_grid_class *remap_grid)
{
	for (int i = 0; i < remap_grids.size(); i ++)
		if (remap_grids[i] == remap_grid)
			return remap_grid;

	for (int i = 0; i < temp_grids.size(); i ++)
		if (temp_grids[i] == remap_grid)
			return remap_grid;
...
void Remap_grid_mgt::prune_grid(Remap_grid_class *remap_grid)
{
	for (int i = 0; i < remap_grids.size(); i ++)
		if (remap_grids[i] == remap_grid) {
			remap_grids.erase(remap_grids.begin()+i);
			break;
		}

	for (int i = 0; i < temp_grids.size(); i ++)
		if (temp_grids[i] == remap_grid) {
			temp_grids.erase(temp_grids.begin()+i);
			break;
		}
}
