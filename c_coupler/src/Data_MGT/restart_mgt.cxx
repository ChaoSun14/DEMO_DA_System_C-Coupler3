...
void Restart_mgt::do_restart_write(const char *annotation, bool bypass_timer, bool bypass_imported_fields)
{
    ...
    int io_proc_mark, io_proc_stride, pio_proc_num;
    Restart_buffer_container *time_mgr_restart_buffer;
    std::vector<Field_mem_info*> imported_fields;
    char restart_data_file_name[NAME_STR_SIZE];
    MPI_Comm io_comm;

    all_H2D_remapping_wgt_files_info->search_wgt_file_info("NULL");
    ...
    if (bypass_timer || time_mgr->is_restart_timer_on()) {
        ...
        int num_io_procs = datamodel_mgr->get_comp_PIO_proc_setting(comp_node->get_comp_id(), io_proc_stride, io_proc_mark, io_comm);
        if (io_proc_mark == 1)
#else
        if (local_proc_id == 0) 
#endif
...
#ifdef USE_PARALLEL_IO
            restart_write_data_file = new IO_pnetcdf(comp_node->get_comp_id(), num_io_procs, io_proc_mark, io_comm, restart_data_file_name, restart_data_file_name, "w", false);
#else
        ...
        for (int i = 0; i < restarted_field_instances.size(); i ++) {
            Field_mem_info *output_field;
            output_field = restarted_field_instances[i].first;
            if (!bypass_imported_fields)
                write_restart_field_data(output_field, NULL, NULL, false);
...
{
    //Field_mem_info *global_field = fields_gather_scatter_mgr->gather_field(field_instance, restart_write_data_file);
    Field_mem_info *global_field;
    ...
    all_H2D_remapping_wgt_files_info->search_wgt_file_info("NULL");
#ifdef USE_PARALLEL_IO
    int io_proc_mark, io_proc_stride;
    MPI_Comm io_comm;
    IO_pnetcdf *active_restart_write_data_file = NULL;
    int num_io_procs = datamodel_mgr->get_comp_PIO_proc_setting(comp_node->get_comp_id(), io_proc_stride, io_proc_mark, io_comm);
    if (io_proc_mark == 1) {
        ...
        active_restart_write_data_file = restart_write_data_file != NULL? restart_write_data_file : backup_restart_write_data_file;
        EXECUTION_REPORT_LOG(REPORT_LOG, comp_node->get_comp_id(), true, "Write variable \"%s\" into restart data file \"%s\"", field_IO_name, active_restart_write_data_file->get_file_name());
    ...
    global_field = fields_gather_scatter_mgr->gather_field(field_instance, NULL, (void*)active_restart_write_data_file, field_IO_name);
#else
	IO_netcdf *active_restart_write_data_file = NULL;
    ...
    global_field = fields_gather_scatter_mgr->gather_field(field_instance, NULL, (void*)active_restart_write_data_file, field_IO_name);
#endif
    global_field->check_field_sum(report_internal_log_enabled, true, hint);
}
