...
if ( $CUSTOM_SETTINGS_GRID_INFO == "5km" ) then
    cp -rf $MODEL_DATA_DIR/ocn*5km.nc ./ocn_demo.nc
    echo "5KM Grid used"
else if ( $CUSTOM_SETTINGS_GRID_INFO == "10km" ) then
    cp -rf $MODEL_DATA_DIR/ocn*10km.nc ./ocn_demo.nc
    echo "10KM Grid used"
else if ( $CUSTOM_SETTINGS_GRID_INFO == "20km" ) then
    cp -rf $MODEL_DATA_DIR/ocn*20km.nc ./ocn_demo.nc
    echo "20KM Grid used"
else if ( $CUSTOM_SETTINGS_GRID_INFO == "50km" ) then
    cp -rf $MODEL_DATA_DIR/ocn*50km.nc ./ocn_demo.nc
    echo "50KM Grid used"
else
    echo "No grid file corresponding to the grid_info("$CUSTOM_SETTINGS_GRID_INFO"), please check!"
endif
cp -rf $MODEL_DATA_DIR/ocn_demo.nml ocn_demo.nml
...
$MYPATH/set_namelist num_3d_vars $CUSTOM_SETTINGS_NUM_3D_VARS
